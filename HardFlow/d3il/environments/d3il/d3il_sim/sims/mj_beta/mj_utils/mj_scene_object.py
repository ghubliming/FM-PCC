...
import d3il.environments.d3il.d3il_sim.sims.mj_beta.MjLoadable as mj_load
...
class CustomMujocoObject(mj_load.MjXmlLoadable):
    def __init__(self, object_name, object_dir_path, pos, quat, root=sim_path.D3IL_DIR):
        if pos is None:
            pos = [0, 0, 0]
        else:
            assert len(pos) == 3, "Error, parameter pos has to be three dimensional."
