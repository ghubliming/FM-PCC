...
from d3il.environments.d3il.d3il_sim.controllers.Controller import (
    ModelBasedFeedforwardController,
)
from d3il.environments.d3il.d3il_sim.core import RobotBase
from d3il.environments.d3il.d3il_sim.sims.mj_beta.mj_utils.mj_helper import (
    IncludeType,
    get_body_xvelp,
    get_body_xvelr,
)
from d3il.environments.d3il.d3il_sim.sims.mj_beta.MjCamera import MjInhandCamera
...
class MjRobot(RobotBase, MjIncludeTemplate):
    ...
    def receiveState(self):
        ...
        self.current_c_vel, _ = self._localize_cart_coords(
            self.current_c_vel_global + self.base_position
        )
    ...
    def get_command_from_inverse_dynamics(self, target_j_acc, mj_calc_inv=False):
        if mj_calc_inv:
            self.scene.data.qacc[self.joint_vel_indices + self.gripper_vel_indices] = (
                target_j_acc
            )
