...
import d3il.environments.d3il.d3il_sim.utils as utils
from d3il.environments.d3il.d3il_sim.controllers import ControllerBase
...
class TrajectoryTracker(ControllerBase):
    ...
    def initController(self, robot, maxDuration):
        ...
        self.startingTime = robot.time_stamp
...
class GotoTrajectoryBase(TrajectoryGenerator):
    def __init__(self):
        ...
        self.desiredPosition = np.array([0, 0, 0, -1.562, 0, 1.914, 0])
...
class GotoTrajectorySpline(GotoTrajectoryBase):
    ...
    def generate_trajectory(self, trackingController, robot, duration):
        ...
        time = np.linspace(0, duration, int(duration / robot.dt) + 1)
        trajectory = np.zeros((time.shape[0], trackingController.dimSetPoint))
