...
Yes, it is entirely possible to use a camera in the "Avoiding" task! In fact, the code confirms that **the avoiding task environment is already configured to include a camera**, but the currently implemented training pipeline (data loader + model) uses state-based observations instead of images by default.

### What is currently implemented:
In `environments/d3il/envs/gym_avoiding_env/gym_avoiding/envs/avoiding.py`, the `ObstacleAvoidanceEnv` explicitly defines and adds a camera to the scene:
...
class BPCageCam(MjCamera):
    def __init__(self, width: int = 1024, height: int = 1024, *args, **kwargs):
        super().__init__(
            ...
            width, height,
            init_pos=[1.05, 0, 1.2],
            init_quat=[0.683, 0.183, 0.183, 0.683], # Looking with 30 deg to the robot
            ...
        )

# Inside ObstacleAvoidanceEnv.__init__:
self.bp_cam = BPCageCam()
self.scene.add_object(self.bp_cam)
self.cam_dict = {"bp-cam": CamLogger(scene, self.bp_cam)}
```
Furthermore, the data recording script (`environments/d3il/gamepad_control/record_data.py`) also links up `cam_loggers=env.cam_dict`. This means if the dataset for avoiding was recorded, **the camera image frames were logged and saved!**

### Why aren't images used currently in Avoiding?
Even though the environment and the data recorder capture the images:
1. **The Environment Output:** The `get_observation()` function in `ObstacleAvoidanceEnv` currently only returns `self.robot_state()[:2]`, which are the (x, y) coordinates of the robot. 
2. **The Dataset Loader:** `Avoiding_Dataset` parses the `data_directory` and explicitly constructs observations as a 1D tensor consisting purely of the `robot_des_pos` and `robot_c_pos` (total 4 dimensions).

### How to switch Avoiding to Vision-Based:
To use the camera in the avoiding task (similar to how they do it in `sorting_4_vision`), you would need to:
1. **Modify/Create a Dataset Loader:** Create an `Avoiding_Vision_Dataset` that loads the `.png` or `.mp4` camera logs instead of just the `.pkl` states.
2. **Change the Observation config:** Update your hydra config (e.g., `configs/avoiding_config.yaml`) to use an image-based agent config (like `agents: ddpm_encdec_agent` instead of `bc_agent`), which relies on `MultiImageObsEncoder` to encode the raw pixels.


---
# sorting_4_vision
The good news: **d3il already has image-based (visual) policy support built in**, and several tasks including Avoiding already have visual model scripts. You don't need to rebuild from scratch.

## What Already Exists in d3il

The official `ALRhub/d3il` repo on GitHub includes **both state-based and image-based policies** for multiple tasks. The `Agents` directory provides 11 imitation learning methods covering both modalities. Specifically, there are vision-based scripts already implemented under `scripts/sorting_4_vision/` — the `ddpm_encdec_benchmark.sh` example trains an image-based DDPM-ACT on sorting. [github](https://github.com/ALRhub/d3il)

## Closest Starting Point for Avoiding + Vision

The repo structure is your blueprint. Here's the path of least resistance:

1. **Check `scripts/sorting_4_vision/`** — this is the clearest existing visual baseline. The architecture (DDPM with encoder-decoder / ACT backbone) is already wired for image observations.
2. **Copy and adapt** the `sorting_4_vision` scripts to `avoiding_vision/` — you mainly need to:
   - Swap the config: use `configs/avoiding_config.yaml` as base
   - Replace `pushing_dataset.py` / `sorting_dataset.py` with an `avoiding_dataset.py` that loads image observations
   - The `environments/d3il/d3il_sim/` folder already has camera/image infrastructure [github](https://github.com/ALRhub/d3il)
3. **Add a camera observation** in the Avoiding env: the MuJoCo environment supports cameras (referenced in `d3il_sim`), so you can add a top-down or wrist cam rendering to the obs dict

## Key Files to Read

| File | Why |
|---|---|
| `environments/dataset/pushing_dataset.py` | Template for dataset with image obs |
| `agents/bc_agent.py` + `agents/base_agent.py` | Agent interface to implement |
| `simulation/pushing_sim.py` | Template for sim wrapper |
| `scripts/sorting_4_vision/ddpm_encdec_benchmark.sh` | Only existing visual training script |

## Related Work / External Code

The D3IL paper (ICLR 2024) evaluates methods like **DDPM-ACT** and **VAE-ACT** on visual inputs and shows visual representations of Avoiding task solutions in Figure 10. These methods (ACT from `act`, Diffusion Policy from `diffusion_policy`) are already integrated as acknowledged dependencies — so the visual backbone infrastructure is there. [arxiv](https://arxiv.org/html/2402.14606v1)

For a more direct visual avoidance reference, there's also the `fpadula/visualcollisionarm` repo which does end-to-end visual obstacle avoidance for a robotic arm using TD3 + camera images, though it uses Unity rather than MuJoCo. [github](https://github.com/fpadula/visualcollisionarm)

**Bottom line**: run `grep -r "image" scripts/ agents/` in the d3il repo to find all existing image-mode entry points, then clone the `sorting_4_vision` pattern for avoiding. The environment camera code already exists — you just need to wire it into the Avoiding env's observation space.

---
# How the Visual Avoiding Works 
[[(How it works --D3IL) (SS26-Thesis-Flow_matching)]]
...
[[Gen4v1 State (Camera Capture and Storage Policy--How the Visual Avoiding Works --(Visual - Avoiding --Visual --D3IL)) (SS26-Thesis-Flow_matching)]]
