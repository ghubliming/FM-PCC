...
| R3 | ❌ | ~~Alignment on **held-out** contexts, and 50–60 contexts (D3IL uses 60)~~ | **RETIRED 2026-09-20, author's call.** D3IL's sixty contexts are the protocol of a *different* task — D3IL scores an image policy that has only to imitate, with no projection in the loop and no per-context solve, so sixty contexts cost it almost nothing. This thesis's alignment episodes each carry a projection solve at every control step. The ten enumerated contexts are the protocol, they are stated as such, and no claim here is measured against D3IL's number. The held-out half is a separate question and is not tracked here either. | ❌ |
...
| R11 | ❌ | ~~Our own sweep of the projection's assumed sampling time~~ | the subsection it was meant to support was removed at v3.47 | ❌ retired 2026-09-20 |
...
| R24 | ✅ | diffusion $\nfe=2$ panel of `fig:raw-plans` | the eight-panel figure is complete | ✅ closed before v3.47 |
...
| R25 | 🔴 | **CI-MeanFM at the extended D3IL-avoiding protocol**, $\nfe=1,2$, all three rules, seeds 6--10 × 20 episodes per geometry | explicit pending rows in `tab:state-headline` | ⏳; evaluation only, all checkpoints exist |
| R26 | 🔴 | **Diffusion $\nfe=2$ for training seeds 7--10**, followed by the DPCC-protocol and extended evaluations | explicit missing row in `tab:avoiding-dpcc-protocol` | ⏳; four trainings, seed 6 exists |
| R27 | ⚪ optional | **Diffusion under endpoint projection on UAV-corridor**, $\nfe=20$, single/$r$/$c$/$t$ | endpoint projection has never been evaluated on diffusion | ⏳ |

## 2. Analyses of existing data (no runs)

| # | what | where it shows | status |
| :-- | :-- | :-- | :-- |
| D1 | ~~Plan smoothness metric~~ | removed from the draft at the author's instruction | ❌ retired v3.47 |
...
| ~~`sec:setup:protocol:aligning` — fifty contexts pending~~ | **removed at v3.46** | R3 retired |
