...
When an item closes: mark ✅ with the date and the v3 version that absorbed it.

## 1. Runs (cluster)

| # | priority | what | why / where it shows | status |
| :-- | :-- | :-- | :-- | :-- |
| R1 | 🔴 | **Seeds beyond 6 on the quadrotor**, pillars $\nfe=5$ first (2–3 seeds × MeanFlow, flow matching, consistency training × 11 configurations) | every quadrotor claim is single-seed; §6.3 `\provisional`, §6.4 `\provisional` | ⏳ |
...
| R4 | 🟠 | Alignment seeds beyond 6 | single-seed; §6.2 `\guard` | ⏳ |
...
| R8 | 🟡 | Flow-based models at 10 episodes on the current evaluation code | §5.5.1 `\provisional` | ⏳ |
