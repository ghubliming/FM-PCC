...
> **24-09 (v3.92), final check for Chapter 6: the draft has no open data cell.**
> - R44b and R44c are read into the draft (R44 complete); R45a is in since v3.82. Every *pending* cell of Ch 5, Ch 6
>   and the appendix is gone.
> - The `\hole`s left are bookkeeping in Reproducibility: the compute total, and job ids in `tab:corpora`.
> - The `\outdated` and dead appendix blocks are superseded by design. The dead twenty-episode block is replaced by
>   `app:avoiding-twenty`.
> - Optional only; nothing in the draft waits on these:
>   - R43 (flow vs diffusion intermediates);
>   - R45b (the corridor baseline at 792 steps);
>   - the s-curve setpoint-bound per-step run (DA R44bc §7);
>   - D10c (box pose for the aligning path figure);
>   - CI-MeanFM K10 of `tab:avoiding-projectors` at two episodes.
> - No new PENDING file was written (author: "if no, do nth").

> **24-09 (v3.80): R16, R36 and R33 are read into the draft, and so is R44 phase A.** The draft's open data cells are now exactly **R44b** (`tab:uav-scurve-projection`, the three per-step rows on FM at $\nfe=1$; job 26195) and **R44c** (`tab:uav-controller`, three cells; C0 job 26196, C1 after R44b) — both in the s-curve file. Everything else in this table is done or struck.
...
| :-- | :-- | :-- | --: |
| **R45** 🟢 R45a ✅ in the draft (v3.82) | Ch 6 §6.3.2 UAV-corridor — the 396-step budget decides the baseline (42 projected flights still moving at the cap) | **R45a (data only, author's preferred):** re-score with the finish line at the end of the corridor, x = 2.0 m — DA agent; R45b (optional): re-fly the six baseline projected cells at 792 steps; R45c: budget 792 for any later corridor run. Spec: [`PENDING_20260924_uav_corridor_step_cap_R45.md`](PENDING_20260924_uav_corridor_step_cap_R45.md) | R45a none; R45b ≈ 2 h wall |
...
| ~~**R33**~~ ✅ | Ch 6 §6.3.2 UAV-corridor — tilt + hump blocks | ✅ **DONE 24-09** — all 62 jobs, 136 cells, batch `temp/23-09-FULL/24-09-1000/batch_uav_20260924_081422`; analysis [`DA_20260924_corridor_v3.md`](../../../../Data_Analysis/DA_in_Paper/analysis/DA_20260924_corridor_v3.md). **In the draft at v3.80** (Tables 6.11–6.13 + the new altitude table, every cell recomputed from the CSV; `fig:uav-corridor-side` replaces the three placeholder figures) | — |
...
| ~~**R16**~~ ✅ | Table 6.5 three rows | ✅ **DONE 24-09** — jobs 26181–26183, complete; FM K2 0.4257 (6 %), CI-MeanFM K10 0.4247 (6 %), FM K10 0.4224 (7 %). Numbers and reading notes in [`DA_20260924_R16_R36_must_need.md`](../../../../Data_Analysis/DA_in_Paper/analysis/DA_20260924_R16_R36_must_need.md) §3. **In the draft at v3.80** (Table 6.5 + budget prose; CI-MeanFM K100 unmoved corrected 0/10 → 1/10) | — |
...
| ~~**R36**~~ ✅ | Table 6.3 three cells | ✅ **DONE 24-09** — jobs 26184–26186, complete; FM keyed on subfolder `K3_thres1_mpc4_n2_msgR36`. 🔴 validation found the printed CI-MeanFM K10 row is a 20-episode cell (caption says two). Numbers in [`DA_20260924_R16_R36_must_need.md`](../../../../Data_Analysis/DA_in_Paper/analysis/DA_20260924_R16_R36_must_need.md) §4, §5. **In the draft at v3.80** (Table 6.3 + prose; the twenty-episode CI-MeanFM K10 cell is disclosed in caption and dataref — re-running it at two episodes, ~15 min, is the author's call) | — |
...
| **R44** 🟡 | Ch 6 §6.3.3 UAV-s-curve — **phase A ✅ read at v3.80** (jobs 26167–26176, `tab:uav-scurve` complete from tag `p23scgrid`; selection FM at K1, author option (ii) = per-step only); **B1 (26195) and C0 (26196) running, C1 after B1** — `tab:uav-scurve-projection` and `tab:uav-controller` *pending (R44b/c)* | **the s-curve file** [`PENDING_20260923_uav_scurve_R44_raw_first.md`](PENDING_20260923_uav_scurve_R44_raw_first.md): A raw grid now (MeanFM, CI-MeanFM, FM at K 1/2/20, diffusion K20); B projection and C MuJoCo MPC only after the author reads A | ~1–2 h (A) |
...
**24-09 — R16 and R36 are closed; no must-need run is left outside the quadrotor scenes.** Jobs 26181–26186
(serial chain, 23-09 22:26 → 24-09 00:14 UTC) all completed with the intended identity. DA: [`DA_20260924_R16_R36_must_need.md`](../../../../Data_Analysis/DA_in_Paper/analysis/DA_20260924_R16_R36_must_need.md). Both new
batches reproduce every printed cell of Tables 6.3 and 6.5. One pre-existing finding for the author: the printed
CI-MeanFM K10 row of Table 6.3 comes from a **twenty-episode** evaluation (one geometry reads 0.95 = 19/20), while
the caption and guard say two episodes — disclose, or re-run that one cell at two episodes (~15 min).
