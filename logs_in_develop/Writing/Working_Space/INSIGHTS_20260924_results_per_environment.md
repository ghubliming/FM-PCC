...
| C6 | **RQ2:** endpoint projection wins over the hump at K20 with one candidate (0.2 against 1.8 violating steps, 291 against 355 ms). Under the tilt it is a trade-off; at K3–5 per-step is ahead. | `sec:res:uav:projection` | §6.3.2.4 ¶3; §6.4.2 |

## UAV-s-curve (§6.3.3) — final (R44 complete, v3.92)

| # | Insight | Source | Applied |
| :-- | :-- | :-- | :-- |
| S1 | **Two limits, the first the controller's: stability at the second turn.** 54 of the 57 lost flow-model flights invert, 48 of them at x 0.2–1.2 m, just past the crossover, 11.6–15.9 s in. On the selected plans MuJoCo MPC removes the inversion (0/10 against 1/10) and brings every flight within 0.3 m of the goal point. *(v3.91: corrects the v3.90 row, which read the 0.30–0.33 m "tracking error" as the cause of the violations.)* | `sec:res:uav:scurve:models`, `sec:res:uav:controller` (DA R44bc §3, §5) | §6.3.3.1, §6.3.3.3, §6.3.3.4 |
