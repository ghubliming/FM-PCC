...
%    Ch 4  all the mathematics, grouped: formulation, overview, plant dynamics and
%          control, generative models, constraint projection, environments
%          (reordered in v2.24; "Low-Level Control" is no longer a section of its own)
...
%    copy is CROSS_STATE.json in this folder. v2 owns Ch 1-4, the abstract, the
...
\begin{description}
  \item[RQ1 --- Generative model.] Under an identical backbone, projection, dataset and evaluation
    ...
    field --- reach the task success and the constraint satisfaction of the diffusion model it
    replaces at fewer solver steps and less planning time per control step, and does the number of
    steps at which it does so order the three objectives?
...
\begin{enumerate}
  ...
  \item \textbf{A quadrotor benchmark.} The controller is carried to an underactuated, open-loop
    ...
    of increasing difficulty. It is where the constraint mechanism is asked to hold on a plant that
    cannot stand still: the manipulator absorbs a bad plan by stopping, the quadrotor flies it out
    (\autoref{sec:bg:quadrotor}). Apart from the vehicle model, everything in this benchmark was
...
Few-step generation is a development inside the transport family of \autoref{sec:bg:fm}, not a
property of generative models in general. The reason is the structure flow matching gives the
problem: the model is trained against a chosen interpolation between the prior and the data, and the
straighter that interpolation is, the fewer integration steps its \ac{ODE} needs. Optimal-transport
paths \parencite{lipman2023flow} and rectified flow \parencite{liu2023flow} pursue exactly this, and
the step budget stays a solver argument throughout. Diffusion admits no such handle: its path is the
noise schedule it was trained with, and a short budget is recovered afterwards, by a different
sampler \parencite{song2021denoising} or by distillation \parencite{wang2024onedp}.
...
For diffusion the step budget is therefore fixed at training time, for instantaneous-velocity matching
it is chosen at deployment, and the two average-velocity objectives make a small budget accurate.
...
a conditioning vector. Transformer backbones operate on image patches instead of a
...
What separates the two platforms for a controller that plans is what a bad plan costs. A manipulator
can stop. Holding the last setpoint costs it nothing, the arm stays where it is, and the clearance it
had it keeps, so a plan that is wrong is absorbed between the planner and the joints. A quadrotor has
no such state, because staying in place is itself a flight. An error in the commanded position is not
held but flown out, over metres of travel, and an attitude that is not recovered ends the flight on
the floor. The planner is the same on both; the margin for error is not.
...
\begin{table}[htpb]
  ...
  \begin{tabularx}{\linewidth}{lLLL}
    ...
    This thesis
      ...
      & receding horizon, horizon $8$, $1$ executed, Euler plant model
      & temporal U-Net, $4.0$\,M parameters for every generative model \\
    \bottomrule
  \end{tabularx}
\end{table}
...
The surrogate gives an end-effector plan and a projection that transfer unchanged from a planar arm to
...
end, which is the end their interval is anchored against. The distributions and their parameters
...
\begin{figure}[htpb]
  ...
  \parencite[Fig.~11]{li2025hardflow} under the Creative Commons Attribution~4.0 licence; the task,
...
\begin{itemize}
  ...
  \item the action bound.
\end{itemize}
Walls and disks are inflated by the vehicle's \emph{rotor reach} $r\sidx{b}$ --- the largest distance
from the centre to a rotor tip along a body axis --- so that a plan drawn for the vehicle's centre
keeps its body clear. That offset is carried by every surface in every geometry, and it is not the
tightening \eqref{eq:method:dpcc:tighten}: the two are separate quantities, applied for separate
reasons, and only the tightening is varied as an experimental factor. The empty scene carries no constraints and serves as the unconstrained reference.
\srcnote{\texttt{config/uav\_projection.yaml}: \texttt{corridor\_hg}, \texttt{pillars\_hg},
\texttt{s\_curve\_hg} (\texttt{inflation.r\_drone} and the per-scene \texttt{planning\_inflation}
that overrides it for the planner, per-segment \texttt{x\_active}); model
