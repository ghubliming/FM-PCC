...
% -----------------------------------------------------------------------------
%  CURRENT SCOPE -- v2.27, 2026-09-23, audit corrections by ChatGPT (Codex)
...
%    This file is current against  v3.60  (2026-09-21)  and  v4: not started.
%    Every cross-draft note addressed to v2 up to that version has been applied
%    or answered; the ledger is ../cross_draft/INBOX.md, and the machine-readable
%    copy is CROSS_STATE.json in this folder. Open as of v2.26 (2026-09-23): the
%    v3.69 row (sec:res:fewstep wording), held for the claims rebuild that the
%    audit of 2026-09-23 asks for (audit from chatgpt/, section 8). v2 owns Ch 1-4, the abstract, the
...
\begin{enumerate}
  ...
  \item \textbf{Endpoint projection against DPCC's per-step projection.} The central contribution of
    ...
    projection after sampling. At the default activation threshold, the smallest budgets provide
    terminal projection only. With active guidance, the evaluated avoiding comparisons reduce
    planning time relative to iterate projection; the outcome also depends on the model and budget
    (\autoref{sec:res:constraints}).
    \srcnote{v3.75 \texttt{chapters/06\_results.tex}, \texttt{tab:avoiding-projectors} and
    \texttt{sec:res:constraints:degenerate}. Corridor comparisons remain pending.}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{A vision-conditioned alignment task.} Once established on the state-based benchmark
    ...
    In the evaluated training contexts, analytic average-velocity matching moves the object closer
    to its target than instantaneous-velocity matching and the diffusion baseline
    (\autoref{sec:res:visual}).
  \item \textbf{A quadrotor benchmark.} The controller is carried to an underactuated, open-loop
    ...
    observations in three scenes that ask three different questions. UAV-pillars is the
    obstacle-avoidance benchmark itself in the air: its field is scaled into an arena of pillars, and
    the planner of the first four contributions flies there unchanged, so the scene reads what the
    vehicle adds to a plan the table has already judged. UAV-corridor is the three-dimensional test:
    ...
    \parencite{todorov2012mujoco}, inside the environment code of D3IL; sampling-based MuJoCo
    predictive control \parencite{howell2022predictive} is the second tracking controller, used in
    the controller comparison alone.
    On UAV-pillars, the projected flights satisfy the declared constraints while contacts with
    pillars outside those constraints still reduce task success (\autoref{sec:res:uav}).
    \srcnote{v3.75 \texttt{chapters/06\_results.tex},
    \texttt{sec:res:uav:pillars}, and \texttt{DA\_20260923\_pillars\_v2\_live.md}.}
\end{enumerate}
...
aerial tasks. \autoref{ch:discussion} interprets the findings and their limits, and
\autoref{ch:conclusion} concludes with the research questions and future work.
...
HardFlow is evaluated on the same obstacle-avoidance task as \ac{DPCC}, with the constraint geometry
of \parencite{romer2025diffusion}, so its differences from this thesis lie in the control loop rather
than in the task: it plans a horizon of sixteen steps and executes eight of them before replanning,
...
The MuJoCo predictive controller is evaluated separately in the s-curve controller comparison
...
cascade from those tracked by MuJoCo predictive control. The per-scene choices are recorded in
...
The planner, its plan, its observation, its projection and its constraint rows are those of the table:
...
and becomes the measured half of the next observation. A contact with a pillar ends the episode as a
failure, as the rod's contact with a post does on the table. The scene has no demonstrations of its own:
...
\texttt{scene.py} $\to$ \texttt{scene\_avoiding\_pillars\_s36.xml}. Scene of record since v3.68; the
