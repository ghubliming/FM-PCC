...
% -----------------------------------------------------------------------------
% ABSTRACT -- v2.10. An abstract, not a story: problem, approach, settings,
% results, contributions. Budget: stay under ~200 words once numbers land.
% -----------------------------------------------------------------------------
\ifstandalone
Diffusion Predictive Control with Constraints (DPCC) plans robot trajectories with a denoising
diffusion model and enforces dynamics and safety constraints by projecting each denoising step onto
a feasible set. Its number of network evaluations is fixed at training time. This thesis replaces
the diffusion model with flow matching and MeanFlow, which generate trajectories by integrating an
ordinary differential equation, so the number of evaluations is chosen at deployment. Backbone,
projection, data and evaluation protocol are identical to the baseline. The method is evaluated on a
planar manipulation benchmark, on a vision-conditioned manipulation task in which two camera views
are encoded into a latent vector, and on a quadrotor whose planned positions are tracked by a
cascaded geometric controller. \hole{Headline results against the DPCC baseline: success rate,
constraint satisfaction, network evaluations, wall-clock time. Keep the abstract under
$\approx 200$ words.} The thesis further compares projecting the sampler's current iterate with
projecting its predicted endpoint, derives the step budget below which the latter reduces to
projecting a finished sample, and reports two negative results.
\fi
...
architecture fixed and varies two parts of it: the generative engine and the projection method.

Comparing engines requires the same backbone, parameter count, data, normalisation, horizon, seeds,
projection and evaluation code. Comparing projection methods requires the same feasible set, solver,
activation threshold and number of candidate plans. Most of the engineering in this thesis keeps
those conditions true.

\section{Research Questions}\label{sec:intro:rqs}

\begin{description}
  \item[RQ1 --- Engine.] Under an identical backbone, projection, dataset and evaluation code, does
...
The thesis makes seven contributions: two performance results, two constructions, and three
methodological or negative results.
...
\begin{enumerate}
  \item \textbf{An architecture-matched engine substitution.} Deterministic transport
    ...
    fixed, so that the engine is the only moving part. Each
    comparison reports success rate, constraint satisfaction, network evaluations and wall-clock
    time together.
  ...
  \item \textbf{A measured regime split between the two projection methods, with a mechanism.}
    The two methods swap places as a function of the same quantity, and the swap follows from
    \emph{which point} each hands to an identical projection operator.
...
\begin{itemize}
  \item \textbf{Step budget $K$.} The number of network evaluations spent producing one plan, also
    written \ac{NFE}. For flow matching it is a solver setting; for diffusion it is fixed by
    training. It is distinct from the number of candidate plans drawn per control step, $B$, which
    is also varied.
  \item \textbf{Iterate projection and endpoint projection.} The two projection methods compared in
    this thesis. Both apply the same projection onto the same feasible set. Iterate projection
    applies it to the sampler's current iterate; endpoint projection applies it to the clean plan
    the sampler is predicted to reach and pulls the correction back into the sampler. Plans
    generated with neither are \emph{unguided}.
  \item \textbf{Genuine steps.} The sampler steps, before the final one, at which endpoint
    projection acts. With none, it reduces to projecting a finished plan
    (\autoref{sec:method:degenerate}).
  \item \textbf{Tightened geometry.} The feasible set shrunk by a margin that absorbs model mismatch
    (\autoref{sec:method:dpcc}).
\end{itemize}
...
A generative planner learns trajectories from demonstrations and produces them by transporting
noise to data. The background follows that order: learning trajectories from demonstrations,
diffusion, flow matching, MeanFlow, and the constrained sampling problem the planner has to solve.
...
execution. The benchmarks come from \parencite{jia2024towards}, whose demonstrations are
multimodal. Planning over a horizon is what makes constraint enforcement possible: a single action
has nothing to project. Multimodality is why the model is generative and not a regressor, and it is
also why a truncated sampler risks averaging between modes.
...
step count is a \emph{training} key, the flow-matching engines' an \emph{inference} key. \srcnote{\texttt{config/}:
...
\begin{itemize}
  \item \textbf{It contains flow matching as the case $r = \ftime$.} The second term of
    ...
    consistency-training variant of \autoref{sec:method:engine}, which is how a set of earlier runs
...
\subsection*{Consistency training on the same objective}

$\alpha$-Flow \parencite{zhang2025alphaflow} keeps the MeanFlow objective but forms its
...
\parencite[\S1,\,\S3]{zhang2025alphaflow}. This thesis uses it as an ablation of MeanFlow
(\autoref{sec:res:fewstep}). \hole{Its training target is not yet written out.}

\subsection*{Where the step budget is set}

For diffusion, $\nfe$ indexes the noise schedule $\beta_{1:\nfe}$ that
\eqref{eq:bg:ddpm:forward} is defined by, so it is fixed when the model is trained. For flow
matching, $\nfe$ is the grid on which \eqref{eq:bg:fm:ode} is integrated, so it is chosen when the
model is used. MeanFlow then removes the discretisation error that made a small $\nfe$ costly.

\section{Model Predictive Control and Constrained Sampling}\label{sec:bg:mpc}

Two ideas meet in this thesis. The first is receding-horizon control \parencite{morari1999model}: an
...
plan admissible but cannot make it more successful. Both projection methods of
\autoref{ch:method} therefore change the learned plan as little as possible. The projection is
...
\subsection*{Iterate projection and endpoint projection}

The two projection methods of this thesis solve \eqref{eq:bg:mpc:proj}--\eqref{eq:bg:mpc:dyn} with
the same $\mathcal{Z}_{f}$, the same solver and the same tolerances. They differ in the point at which
the program is solved.

\begin{itemize}
  \item \textbf{Iterate projection} applies $\proj{\mathcal{Z}_{f}}$ to the sampler's current iterate,
    at selected steps of the generative process. At early transport times that iterate is only
    partly formed and lies far from the data, so the constraints are imposed on positions the
    sampler has not finished producing.
  \item \textbf{Endpoint projection} first forms the clean plan implied by the current iterate and
    its velocity, solves the program there, and pulls the correction back into the sampler before
    the next step \parencite{li2025hardflow}. The program is always solved on something that is
    already a plan.
\end{itemize}

With constraints and solver identical, any difference in cost or quality between the two comes from
where the projection is applied. Endpoint projection needs at least one guided step before the final
one: at the final step the predicted endpoint and the iterate coincide, and the pull-back has no
later step to act on (\autoref{sec:method:degenerate}).

% =============================================================================
\chapter{Related Work}\label{ch:related}
% =============================================================================

Four lines of work lead to this thesis: diffusion planners, flow matching for control, constrained
sampling, and faster generative sampling.

\section{Diffusion-Based Planners and Policies}\label{sec:rel:diffusion}

Diffusion was first used to plan whole trajectories \parencite{janner2022planning}, then to learn
visuomotor policies \parencite{chi2023diffusion}, and then combined with a per-step constraint
projection \parencite{romer2025diffusion}. That last system is both the codebase this thesis starts
from and the baseline it is measured against.

\section{Flow Matching for Robot Control}\label{sec:rel:fm}

Flow matching \parencite{lipman2023flow,liu2023flow} and its few-step variants
\parencite{geng2025meanflow,zhang2025alphaflow} generate samples in far fewer network evaluations
than diffusion, and have been applied to control and manipulation. Most of that work enforces no hard
constraints, so the interaction between a cheap sampler and an expensive projection does not appear
in it.

\section{Safety Filters and Hard-Constrained Sampling}\label{sec:rel:safety}

Constraints on generated samples are enforced by projection
\parencite{power2023sampling,christopher2024constrained,romer2025diffusion}, by control barrier
functions \parencite{yang2025safeflowmatcher}, or by solving an optimisation problem inside the
sampler \parencite{li2025hardflow}. The last of these introduced endpoint projection: projecting the
plan the sampler is predicted to reach, instead of its current iterate. These methods act only in the
later sampling steps. At a small step budget that leaves no step before the final one, and the method
reduces to projecting a finished sample --- a regime that, to the author's knowledge, has not been
characterised.

\section{Accelerating Generative Planners}\label{sec:rel:accel}

Diffusion sampling has been accelerated by distillation, by consistency training
\parencite{song2023consistency}, by better solvers and caching, and by implicit samplers that shorten
the chain of a trained model \parencite{song2021denoising}. These accelerate a model after training.
MeanFlow changes what is learned instead, and leaves the step budget adjustable at deployment.

\section{Positioning of This Thesis}\label{sec:rel:positioning}

Fast generative sampling and hard constraint enforcement have been studied separately. In a
receding-horizon controller the cost sits in their interaction, which has not been measured under
matched conditions. This thesis measures it, and adds three results: the genuine-step condition, the
regime split between the two projection methods, and the removal of redundant candidate-selection
rules.

% =============================================================================
\chapter{Method}\label{ch:method}

The method starts from Diffusion Predictive Control and changes two parts of it: the generative
engine, and one of the two projection methods. Everything else is inherited unchanged, so the
comparisons stay controlled.

The description goes from the bottom up. After the inherited system, the notation and an overview
comes the control layer: the plant model and the controllers that turn a plan into actuator commands
on the manipulator and on the quadrotor. Then the generative model: the three engines, and the
backbone that lets them condition on images. Then the projection: iterate projection, endpoint
projection, the condition under which they differ, and candidate selection.

\section{Starting Point: Diffusion Predictive Control}\label{sec:method:dpcc}

The inherited system \parencite{romer2025diffusion} combines a temporal U-Net backbone
\parencite{janner2022planning}, a diffusion engine, a per-step projection onto a feasible set defined
by dynamics and geometry, several candidate plans with a selection rule, and receding-horizon
execution.
...
\subsection*{Candidate plans and selection rules}
...
At every control step an observation conditions the generative model, the model proposes a plan,
a projection makes the plan admissible, and the first action is executed. The engine and the
projection method are the two parts varied in this thesis; everything else is fixed.
...
All three environments share one control structure: a plant, a first-order model of it that the
projection is written against, and a controller that turns a plan into actuator commands. The
first-order model becomes the equality constraints of \autoref{sec:method:proj}, and the position
increments below are what the engines of \autoref{sec:method:engine} produce.
...
Three engines replace the diffusion engine behind one interface: flow matching, MeanFlow, and
$\alpha$-Flow. Only the training target and the integration loop differ between them.
...
\subsection*{Engine 1 --- flow matching}
...
\subsection*{Engine 3 --- $\alpha$-Flow, consistency training}

$\alpha$-Flow applies consistency training to the MeanFlow objective. With the architecture matched
...
has no such term. Both this collapse and the $r=\ftime$ collapse of \autoref{sec:bg:fewstep} are
extreme hyperparameter values that silently change which engine is measured, and both are checked by
assertion.

\subsection*{Two facts that are not free choices}

\begin{remark}[The prior scale differs between engines]\label{rem:prior}
...
standard deviation at each reverse step. The flow-matching engine inherits the same
...
between engines is a confound the matched architecture is meant to exclude, and it has already caused
one measurable defect here, when a projection method was ported between generations with the
wrong scale.
...
\eqref{eq:method:backbone:film} is FiLM. In this work's code both variants are selected by a flag named
...
names, not the flag values}, and every table names the variant by mechanism.
...
Iterate projection is inherited from DPCC. Below are the program it solves, the schedule on which it
runs, and the point at which it applies the projection.
...
Endpoint projection was introduced by \parencite{li2025hardflow} and is re-derived here on this
work's own constraint set. The same feasible set
...
transport. Two departures from the source keep the comparison controlled: the program is built from
the same constraint list as iterate projection, not from the source's own hard-coded geometry, and the
optional terminal cost $C(\cdot)$ is dropped. Both methods then solve
...
as a black box outside the solver).} Compared with iterate projection, constraints and solver
are identical and only the linearisation point differs, so any cost difference comes from the
algorithm.

Endpoint projection evaluates the network once more per guided step to form the endpoint estimate,
so the two methods are not evaluation-matched even at equal step budgets. Every timing figure in
\autoref{ch:results} carries that extra evaluation.

\subsection*{What differs from the published configuration}

Endpoint projection here is the published algorithm running inside this thesis's apparatus. The experimental
...
\begin{table}[htpb]
  \caption[HardFlow: published setup vs.\ this work]{Endpoint projection as published
  \parencite[App., \emph{Experiment Details}: Robotic Manipulation]{li2025hardflow} against endpoint
  projection as run here. Only the activation convention agrees, so the analysis of
  \autoref{sec:method:degenerate} applies to both.}
...
Four of these rows change what the method does; each follows from keeping the two projection methods
identical everywhere except the point they project.
...
\begin{itemize}
  \item \textbf{Plan length and replanning ($16/8$ against $8/1$).} The published experiment plans
    ...
    which every other method in this thesis also follows. Both halves of that difference matter. The
  ...
  \item \textbf{The dynamics rows.} The source fits a linear model
    ...
    \eqref{eq:method:dpcc:euler} instead, because iterate projection is defined on it. Two methods with different
  ...
  \item \textbf{The solver.} SLSQP throughout, rather than the source's interior-point solver, for
    the same reason: it is the solver of iterate projection. The activation gate that decides whether the algorithm
    runs at all is solver-agnostic, which is why the solver stays out of the method's name.
  \item \textbf{The terminal cost, and what its removal costs.} The source's formulation admits an
    ...
    constraints. This work drops it. Iterate projection has no such term --- the inherited projection
    \eqref{eq:bg:mpc:proj} is pure proximity --- and an endpoint projection with a goal-seeking objective would win
    ...
    minimiser is independent of its weight, so the per-step program of endpoint projection is \emph{exactly}
    $\proj{\mathcal{Z}_{f}}(\hat{\vect{x}}_{1})$: the operator of iterate projection, at a different point. Identical
    constraints, identical solver, different linearisation point is then literally true, and what
    distinguishes endpoint projection is the lookahead and the damped pull-back. And the source's headline on this
    benchmark --- reaching the target in \emph{fewer} steps than unguided sampling --- comes from
    $C$: in its own table every projection-only baseline \emph{lengthens} the path, and only the method
    carrying $C$ shortens it. Endpoint projection as run here has no mechanism that shortens paths,
    so a null task-performance result against iterate projection says nothing about the published
    method.
\end{itemize}

The activation convention is the one row that agrees, and it is the one that matters most. The published
...
\eqref{eq:method:proj:gate} that gates endpoint projection admits a regime in which the only active step is the
...
executes is projection applied to a finished sample: the post-processing baseline.
...
\begin{tabular}{lll}
  ...
  $\ge 2$    & \textbf{admissible} & a claim about the method. \\
  \bottomrule
\end{tabular}
\end{center}
...
The rule invalidates several of this work's own earlier measurements --- every low-budget
endpoint-projection row at the shipped threshold, reported before the boundary was derived. Those
...
Several candidate plans are drawn per control step and one is executed. The three selection rules are random choice, temporal
