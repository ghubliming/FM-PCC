...
\begin{enumerate}
  \item \textbf{Flow matching in place of diffusion.} The diffusion model of \ac{DPCC}
    ...
    evaluation protocol are kept, so the generative model is the only component that changes and the
    number of network evaluations becomes a choice made at deployment. The comparison is made first on
...
such a vision pipeline for its tasks, in which ResNet encoders process each camera view. The features
can be combined with the time embedding of the network, or they can modulate its intermediate
activations through a learned feature-wise scale and shift, FiLM \parencite{perez2017film}. Transformer
backbones operate on image patches instead of a pooled feature vector \parencite{peebles2022dit}.
...
denoising network on image features \parencite{chi2023diffusion}. The conditioning ranges from
feature-wise linear modulation, FiLM \parencite{perez2017film}, to transformer backbones that operate
...
projection method are the two parts varied in this thesis. Comparing generative models requires the
same backbone, parameter count, data, normalisation, horizon, seeds, projection and evaluation code;
comparing projection methods requires the same feasible set, solver, activation threshold and number of
candidate plans. Everything else is shared.
...
behind one interface; only the training target and the integration loop differ between them.
...
of the other two. The three lines are the whole difference between the models.
...
\subsubsection*{Two facts that are not free choices}

\begin{remark}[The prior scale differs between generative models]\label{rem:prior}
...
standard deviation at each reverse step. The flow-matching model inherits the same
$\tfrac{1}{2}$ prior scale at sampling time while its training draws $\vect{x}_{0}\sim
\Normal(\vect{0},\Id{n})$; the MeanFlow model uses unit scale at both ends. A prior-scale mismatch
between models is a confound the matched architecture is meant to exclude, and it has already caused
...
baseline. For the vision-conditioned
task, perception is a vendored encoder producing a fixed-width latent, and the generative
backbone never sees an image. The latent is the interface between perception and generation, so
the claim that the backbone is shared can be checked.
...
\eqref{eq:method:backbone:latent} is therefore identical across every generative model, and everything below it
...
Auxiliary/Naming/TRANSLATION\_20260914\_dev\_jargon\_to\_scientific.md, which is canonical for every name in this thesis.}
\end{remark}

\subsubsection*{Two-time models: the extra input and the JVP constraint}

MeanFlow and consistency training need $\afield_{\theta}(\cdot,\ftime,h)$, so the backbone takes a second time
...
steps, where the prediction \eqref{eq:method:hf:post} is accurate; the published experiments use the
second half \parencite[App., \emph{Experiment Details}]{li2025hardflow}.

\subsubsection*{The instantiation used here}

Two departures from the source keep the comparison with iterate projection controlled. The program
\eqref{eq:method:hf:solve} is built from the same constraint list as iterate projection
(\autoref{sec:method:proj}), not from the source's own geometry, and the terminal cost is dropped,
$C\equiv 0$. The remaining objective is a quadratic proximity term whose minimiser does not depend on
its weight, so
...
solver are identical to iterate projection and only the point of projection differs; the factor
$\ftime_{k+1}$ in \eqref{eq:method:hf:pull}, not the weight, damps the early steps. Each guided step
evaluates the network twice, in \eqref{eq:method:hf:ref} and \eqref{eq:method:hf:pred}, so the two
methods are not evaluation-matched at equal step budgets, and every timing figure in
\autoref{ch:results} carries that extra evaluation.
\srcnote{\texttt{flow\_matcher\_v3\_meanflow/sampling/hardflow\_projection.py:1--40} records the port and
the mode used (\texttt{hardflow\_new}); its four steps are \eqref{eq:method:hf:ref}--\eqref{eq:method:hf:pull}
with $C\equiv0$. The algebra of \eqref{eq:method:hf:post} and \eqref{eq:method:hf:pull} follows from
\parencite[Lemma~1, Algorithm~1]{li2025hardflow} with scheduler $(\ftime, 1-\ftime)$.}

\subsubsection*{What differs from the published configuration}
...
\begin{table}[htpb]
  \caption[Endpoint projection: published setup vs.\ this work]{Endpoint projection as published
...
\subsection{Genuine Steps}\label{sec:method:degenerate}

The activation convention
...
\subsubsection*{The reporting rule, applied to this work's own results}
