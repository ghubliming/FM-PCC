...
USAGE
-----
    python3 bundle/make_bundle.py                 # BOTH variants (below): _new and _full_clean
    python3 bundle/make_bundle.py --annotated-only   # just the annotated _new
    python3 bundle/make_bundle.py --clean-only       # just the clean _full_clean
    python3 bundle/make_bundle.py --full          # the annotated variant with v2 chapters included (_full)
...
THE TWO VARIANTS (author, 2026-09-24, v3.94: "no more 3 versions but one default _new and one clean,
which is no notes at all, just the thesis itself, so I can check the compiled PDF's alignment and format")
---------------------------------------------------------------------------------------------------
    thesis_v3_<stamp>_new.{tex,zip}         ANNOTATED, new sections only (v2 chapters collapsed): every
                                            drafting mark visible -- \srcnote, \dataref, \guard, \hole,
                                            \provisional, \outdated, \flawed, dead blocks. The working copy.
    thesis_v3_<stamp>_full_clean.{tex,zip}  CLEAN, the whole thesis (v2 chapters included, because a
                                            collapsed chapter prints a grey v2 box, which is itself a note):
                                            \srcnote, \dataref, \hole, \outdated and \flawed print nothing;
                                            \guard and \provisional print their text as plain running text
                                            (it is thesis prose); a dead block prints its content without
                                            banner or grey; \todofigure keeps its space. For checking the
                                            layout -- it does NOT show what is missing; the _new build does.
...
# The CLEAN variant (v3.94). \submissiontrue reaches only the two inherited macros (\srcnote hidden,
# \hole warned); every other drafting mark is v3's own, so the switch for them is a \renewcommand here
# rather than a conditional in the preamble. \guard and \provisional keep their TEXT: a guard is the
# caveat sentence that travels with a claim (00_preamble_v3.tex) and a provisional statement is a claim,
# so both are thesis prose; only their colour and brackets are drafting marks.
CLEAN_SWITCH = r"""
% =============================================================================
%  GENERATED BY bundle/make_bundle.py -- CLEAN: THE THESIS ALONE (layout check)
% -----------------------------------------------------------------------------
%  \srcnote and \dataref (provenance) and \hole, \outdated and \flawed (drafting
%  markers) print nothing. \guard and \provisional print their text as plain
%  running text. A dead block prints its content without banner or grey.
...
def build(args, clean=False):
    """One variant. ``clean`` -> the whole thesis with every drafting mark off (CLEAN_SWITCH)."""
    os.makedirs(OUT_DIR, exist_ok=True)
    stamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    full = args.full or clean
    kind = 'full' if full else 'new'
    base = f'thesis_v3_{stamp}_{kind}' + ('_clean' if clean else '')
    ...
    print(f'{"CLEAN, " if clean else "ANNOTATED, "}{"FULL" if full else "NEW SECTIONS ONLY"} -- '
...
def cmd_verify(path):
    ...
    # A block that \input-s another holds the child's lines between its own banners. Fold each DIRECT
    # child back to one token on both sides: in the bundle the child is the blank line flatten() adds,
    # its BEGIN..END banners and the blank line after; in the source it is the \input line itself.
    # (v3.94: before this, a chapter with an \input could never verify.)
    def children_of(a, b):
    ...
    def folded_source(src):
        ...
        for ln in open(src).read().split('\n'):
            ...
            if m.group('pre').strip():
                out.append(m.group('pre').rstrip())
            out.append(f'\x00INPUT {os.path.relpath(child, V3)}')
            if m.group('post').strip():
                out.append(m.group('post').lstrip())
        return out
...
def main():
    ...
    ap.add_argument('--annotated-only', action='store_true',
                    help='build only the annotated variant (_new, or _full with --full)')
    ap.add_argument('--clean-only', action='store_true',
                    help='build only the clean variant (_full_clean): the whole thesis, no drafting marks')
