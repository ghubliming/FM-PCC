...
edited by hand and can never drift from the split one.
    ...
    ../thesis_v3.tex  +  parts/  +  chapters/   ->   bundle/output/thesis_v3_<stamp>.tex
                                                     bundle/output/thesis_v3_<stamp>.zip
...
-----
    ...
    python3 bundle/make_bundle.py --clean-notes   # hide \srcnote in the review PDF
    python3 bundle/make_bundle.py --no-notes      # also hide \dataref and \guard (reading copy)
...
THE THREE NOTE LEVELS
---------------------
    (default)       every drafting macro visible -- the working copy
    --clean-notes   \srcnote hidden; \dataref, \guard, \hole, \provisional visible
    --no-notes      \srcnote, \dataref and \guard hidden; \hole and \provisional visible

\hole and \provisional are never hidden: they mark work that is missing, and a build that
hides those lies about how finished it is. \guard is hidden only by the explicit flag,
because a guard is a CAVEAT and not a note -- some are internal ("this panel was cropped"),
others state a real limit of a real claim ("one training seed, so these counts are not the
rates of Table 6.4"). --no-notes therefore prints how many it hid. It is for a tidy reading
copy, never for a PDF someone is asked to judge the work on.
...
# thesis_v3_<stamp>[_new|_full][_clean][_<n>].tex -- the unsuffixed form is pre-v3.6.
RE_BUNDLE = re.compile(r'thesis_v3_\d{8}_\d{6}(?:_(?:new|full)(?:_clean)?)?(?:_\d+)?\.tex')
...
CLEAN_NOTES_SWITCH = r"""
% =============================================================================
%  GENERATED BY bundle/make_bundle.py -- CLEAN REVIEW COPY
% -----------------------------------------------------------------------------
%  The inherited preamble defines \submissiontrue as the supported switch that
%  hides \srcnote. Holes remain visible and emit warnings by design.
% =============================================================================
\submissiontrue
"""

# --no-notes. \submissiontrue only reaches \srcnote, because that is the one note the
# inherited preamble owns. The other two grey/violet macros are v3's own, so the switch
# for them is a \renewcommand here rather than a conditional in the preamble.
#
# WHY \guard IS NOT LIKE THE OTHER TWO, and why this is opt-in.
#   \srcnote  points at code   -- provenance, internal, never ships.
#   \dataref  points at data   -- provenance, internal, never ships.
#   \guard    is a CAVEAT      -- some of them are internal ("this panel was cropped"),
#                                 but others state a real limit of a real claim ("one
#                                 training seed, so these counts are not the rates of
#                                 Table 6.4"). Hiding those makes the PDF overstate the
#                                 results.
# So the build prints how many it hid and says what that costs. It is a switch for a
# tidy reading copy, never for anything a reader is asked to judge the work on.
NO_NOTES_SWITCH = r"""
% =============================================================================
%  GENERATED BY bundle/make_bundle.py -- NOTES HIDDEN (--no-notes)
% -----------------------------------------------------------------------------
%  \srcnote (code provenance), \dataref (data provenance) and \guard (caveat)
%  render as nothing in this build. \hole and \provisional stay visible by
%  design: they mark work that is missing, and a build that hides those is a
%  build that lies about how finished it is.
%
%  A HIDDEN \guard IS STILL A LIMIT OF THE RESULT. This PDF is a reading copy.
% =============================================================================
\submissiontrue
\renewcommand{\dataref}[1]{}
\renewcommand{\guard}[1]{}
"""
...
def count_macro(lines, macro):
    """How many times the macro is CALLED in the flattened body.

    Comments are stripped first: the preamble documents each drafting macro in a
    comment that spells the call, and counting those would overstate by one per macro.
    """
...
def build(args):
    ...
    base = f'thesis_v3_{stamp}_{kind}' + ('_nonotes' if args.no_notes else
                                          '_clean' if args.clean_notes else '')
    ...
    n_guard = count_macro(lines, 'guard')
    n_dataref = count_macro(lines, 'dataref')
    if args.no_notes:
        lines = inject_shim(lines, NO_NOTES_SWITCH)
    elif args.clean_notes:
        lines = inject_shim(lines, CLEAN_NOTES_SWITCH)
    ...
    print('  review notes: ' + ('none (\\srcnote, \\dataref and \\guard hidden)' if args.no_notes else
                                'clean (\\srcnote hidden)' if args.clean_notes else
                                'annotated (\\srcnote visible)'))
    if args.no_notes:
        print(f'  HIDDEN: {n_guard} \\guard caveat(s) and {n_dataref} \\dataref pointer(s). '
              'A hidden guard is still a limit of the result --')
        print('          this build reads tidier than the evidence is. '
              'Use --clean-notes for anything judged on its claims.')
    if collapsed:
...
def main():
    ...
    ap.add_argument('--clean-notes', action='store_true',
                    help='activate \\submissiontrue in the generated bundle, hiding \\srcnote; '
                         'holes remain visible and warn')
    ap.add_argument('--no-notes', action='store_true',
                    help='everything --clean-notes does, and additionally hides \\dataref and '
                         '\\guard, so no coloured drafting note is left in the text. \\hole and '
                         '\\provisional stay visible by design. The build reports how many guards '
                         'it hid: a hidden guard is still a limit of the result')
