...
\begin{description}
  \item[RQ1 --- Generative model.] With the backbone, projection, data and evaluation code held fixed,
    ...
    any environment (\autoref{sec:res:summary:models}). The budget is part of that frontier: the flow-based
    models keep their success down to one network evaluation on the D3IL-avoiding benchmark, where the
    diffusion model loses it below the budget it was trained with (\autoref{sec:res:fewstep}), and on
    D3IL-aligning analytic average-velocity matching improves with more evaluations while
    consistency-interpolated average-velocity matching does not.
