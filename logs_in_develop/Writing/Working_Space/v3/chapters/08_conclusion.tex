...
instantaneous-velocity matching in every context, and on the quadrotor it passes the goal collision-free on
every flight of UAV-corridor, which the projected baseline never reaches. On UAV-pillars
instantaneous-velocity matching passes the goal collision-free more often, and analytic average-velocity
matching does so in a shorter path. Consistency-interpolated average-velocity matching is level with analytic average-velocity matching where the tasks cannot separate them and behind
...
\begin{description}
  \item[RQ1 --- Generative model.] With the backbone, projection, data and evaluation code held fixed,
    analytic average-velocity matching is ahead of the diffusion baseline in every environment, and ahead of
    instantaneous-velocity matching on D3IL-avoiding, D3IL-aligning and UAV-corridor. On UAV-pillars the two
    trade: instantaneous-velocity matching passes the goal collision-free more often, analytic
    average-velocity matching in fewer control steps. Instantaneous-velocity matching is ahead of the
    baseline on D3IL-avoiding and UAV-pillars and level with it on D3IL-aligning.
    Consistency-interpolated average-velocity matching is not ahead of analytic average-velocity matching in
    any environment (\autoref{sec:res:summary:models}). It reaches that level at fewer solver steps and at
    less planning time per control step: the flow-based models keep their success down to one network
    evaluation on the D3IL-avoiding benchmark, where the diffusion model loses it below the budget it was
    trained with (\autoref{sec:res:fewstep}), and one such evaluation costs a fraction of the twenty the
    baseline spends. The budget orders the three objectives only where the environment is hard enough to
    separate them: on D3IL-avoiding all three hold their success down to one evaluation, whereas on
    UAV-corridor the two average-velocity objectives fly every flight collision-free from three evaluations
    on and instantaneous-velocity matching does not reach that at any budget evaluated, and on D3IL-aligning
    analytic average-velocity matching improves with more evaluations while consistency-interpolated
    average-velocity matching does not.
  ...
  \item[RQ3 --- Transfer.] The lead of analytic average-velocity matching carries over from state to camera observations and from the
