...
same budget are not. On D3IL-aligning it brings the box closer to its target than the baseline and flow
matching in every context, and on the quadrotor it completes UAV-corridor collision-free and leads on
UAV-pillars. Consistency training is level with MeanFlow where the tasks cannot separate them and behind
...
on UAV-pillars, while its constraint result is level on D3IL-aligning, lower on UAV-corridor and higher on
UAV-pillars.
...
\begin{description}
  \item[RQ1 --- Generative model.] With the backbone, projection, data and evaluation code held fixed,
    ...
    is ahead of the baseline on D3IL-avoiding and UAV-pillars and level with it on D3IL-aligning. Consistency training is not ahead of MeanFlow in
  ...
  \item[RQ2 --- Budget.] The flow-based models keep their success down to one network evaluation on the
    ...
    (\autoref{sec:res:fewstep}). On D3IL-aligning MeanFlow improves with more evaluations and consistency
    training does not.
  \item[RQ3 --- Constraints.] Endpoint projection is well-posed only with at least one guiding step
    ...
    and UAV-corridor and projection time on UAV-pillars; its constraint satisfaction is level on D3IL-aligning,
    higher on UAV-pillars and lower on UAV-corridor (\autoref{sec:res:constraints}). D3IL-avoiding is
  ...
  \item[RQ4 --- Transfer.] MeanFlow's lead carries over from state to camera observations and from the
    manipulator to the quadrotor; the step from flow matching to diffusion does not carry over to camera
...
\hole{Refine in v4; results on D3IL-aligning and the quadrotor rest on one training seed.}
