...
- **Bundle (tenth round):** `bundle/output/thesis_v3_20260923_201232_new.zip` + `…201233_new_clean.zip` + `…201233_new_nonotes.zip`, byte-faithful, supersedes `164500`. Not compiled.

## v3.77 — 2026-09-23 · Fig 6.4 drawn complete (a plotting fix); R36's K3 cells moved to seeds 7–10 → [`changelogs/v3.77_20260923_fig64_plotting_fix_R36_seeds.md`](changelogs/v3.77_20260923_fig64_plotting_fix_R36_seeds.md)

- **Fig 6.4** was missing two hollow points whose data exist (CI-MeanFM endpoint: the store read the tagged run without an endpoint arm; diffusion per-step: skipped, and only in the 23-09 batch). Store fixed through `sources.ALIGNING_PROJECTED_EXTRA`, values checked against Table 6.8, the struck-through "box not moved" label moved; rebuilt and exported; Ch 6 sentence, caption and dataref updated with a `%` comment. Frontier unchanged.
- **Scheduled runs:** R16 unchanged; **R36a/b 🔄 updated** to seeds 7–10 (the printed MeanFM K3 row's seeds) in the driver, runbook §6b and ledger; no new run needed. `check.py` passes; no bundle.

## v3.76 — 2026-09-23 · D3IL-aligning: R2, R37 and R35 read into §6.2; the run ledger checked against the draft → [`changelogs/v3.76_20260923_aligning_R2_R37_R35_filled_ledger_checked.md`](changelogs/v3.76_20260923_aligning_R2_R37_R35_filled_ledger_checked.md)

- **§6.2 filled from `DA_20260923_R2fix_R37_aligning.md`** (no printed number moved): Table 6.6 K2 row (from Table 6.7's cell) and K100 η0.1 row (R37b: per-step 8/10, endpoint 8/10 at 0.153 m, nine guiding steps add a fifth to the step); Table 6.8 diffusion per-step rows (R2: 4/1/1 clean, Pareto-dominated by MeanFM under the same rule) and CI-MeanFM endpoint rows (R35, already in the corpus). Prose: endpoint ahead in **seven of nine** comparisons, "ten of ten at both budgets" scoped, obsolete no-baseline guard deleted, held-context guard added, Fig 6.4 text/caption say what is drawn, the aligning conclusion's "In one line" rewritten as facts; two rounded ratios corrected.
- **Ledger checked (item 1):** the draft's open data cells are exactly R16, R36 (both scheduled, specs match the tables), R33 (corridor, 24-09) and R44 (s-curve, phase A submitted); nothing else waits on a run. NOW table's stale R39 row corrected to ✅. No bundle (author). `check.py` passes; not compiled.

## v3.75 — 2026-09-23 · UAV-s-curve: the grid at K = 1, 2, 20; projection and controller pending until the raw grid is read (R44) → [`changelogs/v3.75_20260923_scurve_grid_1_2_20_raw_first_R44.md`](changelogs/v3.75_20260923_scurve_grid_1_2_20_raw_first_R44.md)

- **Table 6.14** is the author's grid: MeanFM, CI-MeanFM, FM at K = 1, 2, 20 and diffusion at 20, ten flights; the K = 10 / K = 5 pilot rows (printed at v3.71 only because they were on disk) moved to the appendix; four cells pending. **Tables 6.15 and 6.16** are pending for "the configuration selected in Table 6.14" — the v3.71 choice of FM K = 2 is withdrawn (endpoint projection has no guiding step at K = 2). Readings of the pending parts are `\hole`s; Ch 5 protocol and the appendix records follow.
- **New run file `data_status/PENDING_20260923_uav_scurve_R44_raw_first.md`:** phase A (raw grid) now, B (projection) and C (MuJoCo MPC) only after the author reads A. Every older s-curve item (22-09: R15, R29, R31, R38, R40; 23-09: R42) deleted or pointed to it.
- **Bundle (ninth round):** `bundle/output/thesis_v3_20260923_164500_new.zip` + `…164500_new_clean.zip` + `…164501_new_nonotes.zip`, byte-faithful, supersedes `163243`. `check.py` passes; not compiled.

## v3.74 — 2026-09-23 · one "Appendix" heading over the lettered appendices → [`changelogs/v3.74_20260923_appendix_part_heading.md`](changelogs/v3.74_20260923_appendix_part_heading.md)

- Author: the appendix should sit under one big "Appendix" heading. None had ever existed (v3, v2 and the template all print A, B, C as top-level chapters); `\addpart{Appendix}` added after `\appendix{}` in `09_appendix.tex`. A, B, C and all `app:` references unchanged. Bundle `thesis_v3_20260923_163243_new.zip` (+ clean, nonotes), byte-faithful, supersedes `160926`. Not compiled.

## v3.73 — 2026-09-23 · v2.26 synced; Chapter 5 brought in line with the pillars result; five audit items; bundle → [`changelogs/v3.73_20260923_v2.26_sync_ch5_pillars_audit_items_bundle.md`](changelogs/v3.73_20260923_v2.26_sync_ch5_pillars_audit_items_bundle.md)

- **Ch 5 after v3.72:** the UAV-pillars scene text states facts instead of the pre-result framing ("a difference is a result about the plant"; "the vehicle adds only … tracking error"); each geometry's keep-out disk covers one of the six pillars and the other five are in no constraint set; contact ends a flight (metric bullet); UAV-pillars is five seeds (seeds sentence); `tab:eval`'s UAV column is "UAV-corridor, UAV-s-curve" with UAV-pillars in the caption (its stale 634-step limit was `pillars_hg`'s); §5.6.3.3 prints the pillars plant constants (1.0 m, 1 s = 100 × 0.01 s, ≤ 1 m/s, no feed-forward). Ch 6: §6.3 intro's "one training seed / 3.96–3.97 M" scoped to corridor and s-curve; "five of the six pillars".
- **v2.24 → v2.26 synced** per file (Ch 5's baseline advanced by hand; owned file not merged); `fig_hardflow_endpoint_generation.png` copied from v2 (byte-identical to the arXiv source).
- **v2's audit note:** items 1 (FM prior 0.5 vs trained 1), 2 (endpoint sampler at h = 0 on the average-velocity models), 3 (per-step gate eleven vs ten), 5 (NFE = K + n_guide), 6 (pillars constants) applied in Ch 5 with srcnotes, one clause in Ch 6 §6.1.2.4; item 4 left to the author. Correction and one check sent to v2; the Ch 7 reading sent to v4. Misfiled v3.72 inbox row moved to → v4.
- **Bundle (seventh round):** `bundle/output/thesis_v3_20260923_160926_new.zip` + `…160926_new_clean.zip` + `…160927_new_nonotes.zip` (10 478 KB, 47/47 figures render, byte-faithful), supersedes `152138`. `check.py` passes; not compiled.

## v3.72 — 2026-09-23 · UAV-pillars landed: §6.3.1 read from the R39 corpus → [`changelogs/v3.72_20260923_uav_pillars_landed_R39.md`](changelogs/v3.72_20260923_uav_pillars_landed_R39.md)

- **§6.3.1 filled from `live_p23uavpv2live`** (jobs 26151–26154, 240 flights, DA of record `DA_20260923_pillars_v2_live.md`): `tab:uav-pillars-raw` air columns (mean ± seed spread), new `tab:uav-pillars-geo` (projected S&C per geometry with contacts), `fig:uav-pillars-paths` in place of the placeholder (MeanFM nfe 1, before/after × three geometries). Reading: before projection the air reproduces the table; after projection the declared constraints hold on every flight (0 violating steps / 78) while S&C falls to 0.60–0.70 because a third of the projected flights end on a pillar — pillars are not in the declared set, rod radius → rotor reach, 50/51 commanded paths within the reach; loss on *both-hard*; models level.
- Conclusion §6.3.5 pillars paragraph written; "The benchmark" pillars half filled (corridor half still R33). Figure-store subtitles fixed (clipped, run-tag name). Ledger R39 ✅; inbox DA→v3 ✅; note → v4. `check.py` passes; not compiled; no bundle.

## v3.71 — 2026-09-23 · UAV-s-curve rebuilt (grid → selection → projection → controller), one controller name, Fig 5.11 without the pillars panel → [`changelogs/v3.71_20260923_scurve_rebuild_controller_name_fig511_no_pillars.md`](changelogs/v3.71_20260923_scurve_rebuild_controller_name_fig511_no_pillars.md)

- **§6.3.3–6.3.4 rebuilt** (rollback copy in `withheld/20260923_v3.71_archive/`): `tab:uav-scurve` is now the budget grid of every model without projection (MeanFM K2/10, CI-MeanFM K1/2/5, FM K2/20, diffusion K20 filled from `u7hg` / `u6unet_ae02`, ten flights each; MeanFM K1/K20, CI-MeanFM K20, FM K1 pending R42a); the model crossing the finish line most often — **FM at K2, 7/10** on the corpus — is the one projected (`tab:uav-scurve-projection`, new: per-step tightened ×3 rules filled, endpoint pending R42b) and flown under the two tracking controllers (`tab:uav-controller`, ten flights per cell, MJPC rows pending R42c). The MeanFM K10 three-flight MJPC pilot stays as the pilot in the text, Fig 6.9 and `tab:uav-controller-cost`.
- **One controller name:** "the cascaded geometric controller" everywhere; the velocity-setpoint paragraph of §6.3 and every "brake-to-rest" (Ch 5/6) removed — the setpoint policies are v2's `eq:method:dep:vdes`; `tab:eval` row renamed; Fig 6.9 panel titles renamed (MuJoCo MPC / Cascaded geometric controller). R40 struck.
- **§5.5.3:** *aborted* defined (the vehicle inverts; attitude only; not a success; step recorded). **§5.6.3.3:** s-curve protocol sentence rewritten.
- **Fig 5.11:** UAV-pillars panel dropped (its demonstrations are the avoiding ones, said in one sentence); two-column layout, caption 72 words. Store + export rebuilt.
- Item 0 answered in the changelog (SVG is the source; pdflatex compiles the PNG; `--svg-package` or `svg2pdf.sh` for vector). Ledger/runbook: R42a/b/c specified, nothing submitted. `check.py` passes; not compiled. **Bundle (sixth round):** `bundle/output/thesis_v3_20260923_152138_new.zip` + `_clean` + `_nonotes` (9836 KB, 46/46, byte-faithful), supersedes `142523`.

## v3.70 — 2026-09-23 · Review fixes, Ch 5: tightening justified by DPCC's numbers, Fig 5.11 matrix, holes closed → [`changelogs/v3.70_20260923_review_ch5_tightening_numbers_fig511_matrix.md`](changelogs/v3.70_20260923_review_ch5_tightening_numbers_fig511_matrix.md)

- **§5.1.2 / §5.5.1:** the tightened set is used because DPCC's own numbers show it is better — Table 1 without/with tightening (S&C 0.49 → 0.98, violations 6.0 → 0.0, steps 76.1 → 69.0) and Fig. 2 (0.41/0.51/0.49 → 0.96/0.99/0.98 across the selection criteria) quoted in §5.5.1; **`tab:target` gained a "Published, no tightening" row**; §5.1.2 forward-links §5.5.1 and points to v2's shrinkage equation. The "adds nothing a reader could rank on" sentence and the corridor aside are gone.
- **§5.3.2:** Fig 5.10 hole closed — the figure is the $x$–$y$ top view of the contexts, not the demonstrated paths. **§5.3.3:** pillars paragraph reduced to facts (two plants, same plans); guard deleted.
- **Fig 5.11** rebuilt as the matrix of Fig 5.8 (pillars | corridor tilt over hump | s-curve, one box); caption five lines, its content in the text. **Table 5.7** row spacing. **§5.6.3.3** budget-grid clause cut.
- **Ch 6, items 7–12:** **captions capped** — every caption over 100 words (2 in Ch 5, 13 in Ch 6) rewritten to ≤ 98, protocol/argument moved to the text; checklist `CAPTION_CHECKLIST_20260923_v3.70.md`. §6.1.1 raw-plans violation counts stated in one verified sentence (MeanFM K1 6.0, K2 12.0; diffusion K1 28.0, K2 **21.5** (was 20.5 — corrected against the corpus), K20 18.5); panel paragraph, K2/K10 baseline paragraph, caveat paragraph and the avoiding conclusion's one-liner rewritten as facts (Pareto: CI-MeanFM and FM dominate the baseline of record; MeanFM trades one episode in thirty). Three slop patterns added to `Writing_Hints/HINT_20260920`.
- **Items 13–14:** the $\alpha_{\mathrm{end}}=0.05$ cells (K2, K20) added to Table 6.5 from the corpus, so §6.2.1.2's claim has its rows; **Table 6.6 rebuilt** — K10/K20 filled from the tightened cells of Table 6.7, K2 and K100 η0.1 pending (R37a/b submitted), **K100 η0.5 "not run"** and priced by the rule of thumb in §6.2.2 (⌈ηK⌉ × 4 solves per control step at the 6–27 ms per solve the two measured cells give → hours per cell); R37c struck in the ledger and runbook. **Bundle (fifth round):** `bundle/output/thesis_v3_20260923_142523_new.zip` + `_clean` + `_nonotes` (10083 KB, 46/46, byte-faithful), supersedes `110037`.

## v3.69 — 2026-09-23 · Review fixes: Ch 5 order (task → constraints), quadrotor figures rebuilt, one seed on pillars, ten corridor flights, diffusion K2 filled → [`changelogs/v3.69_20260923_review_fixes_ch5_order_uav_figures_one_seed_K2.md`](changelogs/v3.69_20260923_review_fixes_ch5_order_uav_figures_one_seed_K2.md)

- **Ch 5:** the "(v3.67) … printed pending" drafting talk cut from §5.2.1 (the grey `[src: …]` is `\srcnote`, annotated bundle only); every task subsection now runs environment/task figure → constraints table → constraint figure (avoiding, alignment, quadrotor); Fig 5.7's three orthographic views redrawn from behind the start (they looked from the finish end, which read as mirrored) and its caption rewritten to facts; **Fig 5.8 is a 3 × 3 matrix** — pillars × three mapped avoiding geometries, corridor × tilt (top) + hump (side), s-curve — all panels one size; §5.5.2 gives the one reason the D3IL metric is not used; **UAV-pillars = one seed, two episodes per geometry; UAV-corridor = ten flights per configuration** (the driver's 12 = 4 per route from U16; DA reads the first ten, 4/3/3); §5.6.3.3 rewritten; no numbered seed anywhere in prose or captions.
- **Ch 6 / appendix:** pillars captions to one seed, live-check guard removed; corridor captions to ten flights; **diffusion K2 five-seed row filled** in Tables 6.1/6.2/A.1/A.2 (DA_20260923; R26 closed) and §6.1.1 says the baseline's cheapest S&C-1.000 configuration is K2 `c` (38 % of K20's cost; 30× → 12× against it). Figure 6.3 still lacks the K2 point (store scripts not yet promoted — needs a go). 🔒 §6.4 corridor rows (`12/12`) untouched.
- **Second round (items 13–17):** §6.1.2.3 executed paths → appendix `app:avoiding-paths` (frontier now follows Table 6.2; conclusion names the appendix); **Fig 6.4 withdrawn** (ragged budgets) and replaced by `tab:k-ladder`, the full $\nfe\in\{1,2,5,10,20\}$ grid with eight cells pending **R41** (evaluation only, ~3 h; diffusion K5 optional); §6.2.1.1 paragraph rewritten as prose; Tables 6.7/6.8 print **median and mean** of the final distance (means from the corpus of record); Fig 6.7 → appendix `app:aligning-paths`; ledger R39 (no live cells) + R41.
- **Third round:** Fig 5.8 corridor column = tilt top + tilt side + hump side (planes named); Fig 5.11 gains a **UAV-pillars panel (the 96 avoiding demonstrations mapped)** and a **hump side panel** with the demonstrated altitude band; §5.5.3 links v2's `sec:method:mjpc`; **UAV-pillars back to five seeds × two episodes** (author's second decision); aligning protocol row corrected (every model 10 contexts × 1 episode — the diffusion cell is one rollout per context on 30 contexts, reduced to ten); **diffusion K2 time withheld pending re-timing (R42, ledger §29)** — identical on all seeds, projection half ~3× per solve, CPU-contention signature of job 26112; §6.1.1 states the cost factor against the cheapest diffusion cell as a bound (4–12×) until R42. `check.py` passes. **Later the same day:** K2 diffusion time kept as measured and explained in a new §6.1.2.5 *Caveat: What the Projector Is Handed* (first-step projection on a near-noise plan; R42 closed without a run, R43 optional test of the smoothness conjecture); *Step Budget* dropped (`sec:res:fewstep` → §6.1.2.1, R41 struck); endpoint-vs-per-step verdict; Fig 6.4 key moved + trade-off sentence; **UAV-pillars rebuilt as a plant check** (MeanFM/CI-MeanFM K1/K2 table-vs-air, arm-vs-quadrotor path figure). **Bundles (fourth round):** `bundle/output/thesis_v3_20260923_110037_new.zip` + `_clean` + `_nonotes` (10120 KB, 46/46, byte-faithful), supersede `103745`. **Bundles (third round):** `bundle/output/thesis_v3_20260923_103745_new.zip` + `_clean` + `_nonotes` (10117 KB, 46/46 figures, `--verify` byte-faithful), supersede `100734`. **Bundles (second round):** `bundle/output/thesis_v3_20260923_100734_new.zip` + `_clean` + `_nonotes`, 46/46 figures, refs resolve, `--verify` byte-faithful; supersede `204547`.
...
- **Second round (23-09):** the corridor is **two constraints read separately** — tilt (the slide leaned −60°) and hump (the roof) — with tilt/hump blocks in every corridor table; all OUTDATED banners removed from the quadrotor parts; `fig_scene_uav_pillars`, `fig_constraints_uav` (pillars = avoiding both-hard mapped at scale 36) and `fig_expert_uav` rebuilt; 23-09 pending file + runbook corrected to the two scenes/tags/driver. **Third round:** keep-out disk defined after Table 5.2 with a new `fig:constraints-avoiding-sets`; Figure 5.6's pillars render made **here** (MuJoCo + osmesa) on the scaled arena, the other two re-rendered with the same code; no placeholder left in Ch 5. **Fourth round:** Fig 5.7 gains side views of the tilt and the hump; §5.5.1 states the tightened set with DPCC's reason; Ch 6: appendix sentence fixed, raw-plan fidelity analogy, §6.1.2.2 opener cut, Figs 6.3/6.6 say which rule is drawn, Fig 6.4 lines displaced, Figs 6.5/6.6 outcome axis log in the distance left, **Table 6.7's K2 endpoint rows removed** (zero guiding steps — the data are the degenerate form, not wrong), seeds never numbered, **§6.3 one part per scene**, sub-conclusions with the model ordering in 6.1.4/6.2.4/6.3.5 and one paragraph in 6.4.3 (author's request under the lock). **Bundles (closing the fourth round):** `bundle/output/thesis_v3_20260922_204547_new.zip` + `_clean` + `_nonotes`, 46/46 figures, refs resolve, `--verify` byte-faithful. Ledger: NOW table + struck runs. **Bundles (second round):** `bundle/output/thesis_v3_20260922_195031_new.zip` + `_clean` + `_nonotes`, 45/45 figures, refs resolve, `--verify` byte-faithful; supersede `162325`.
- Ledger: R31 struck, R38 optional, R40 opened (23-09 file §4); issues for the run agent in §5. `check.py` passes. **Bundles (v3.68 first round):** `bundle/output/thesis_v3_20260922_162325_new.zip` + `_clean` + `_nonotes`, 45/45 figures, refs resolve, `--verify` byte-faithful.
...
- **Ch 6 withdrawn, structure kept:** Table 6.6 → all rows *pending (R37)*, its K100 cost pair and conclusion → holes; the diffusion untightened projected sentence → one line (R2); Table 6.15 s-curve projected cells → *pending (R38)*; corridor tightening paragraph removed (R33 = 68 cells); "untightened set" wording under unprojected tables replaced. Appendix: HF-ladder detail keeps tightened columns only; `tab:tiers` row removed; s-curve projected records pending (R38). Pillars block (disabled) and 🔒 §6.4 untouched.
- **Appendix twenty-episode section marked DEAD** (new `deadblock` environment: banner + grey, nothing inside edited; author will drop or rebuild it).
- Ledger §24; recap MD superseded banner; note to v4. `check.py` passes. **Bundles (closing v3.67):** `bundle/output/thesis_v3_20260922_150138_new.zip` + `_clean` + `_nonotes`, 45/45 figures, refs resolve, `--verify` byte-faithful; supersede the v3.66 set.

## v3.66 — 2026-09-22 · KEY UPDATE: tightened/untightened settled; §6.1.2.6 rebuilt on matched cells; Tables 6.5/6.6/6.8 + Fig 6.7 at the operating point; every UAV-corridor block blanked for the corridor-v3 re-run (R33); ledger renewed → [`changelogs/v3.66_…`](changelogs/v3.66_20260922_key_update_projectors_alignment_tables_corridor_rerun.md)

- **Tightened/untightened:** recap MD (`RECAP_20260922_tightened_vs_untightened.md`) — every projected headline is tightened, nothing to run twice; one bold convention sentence in Ch 5 §5.2.1.
- 🔴 **§6.1.2.6 rewritten:** Fig 6.5 + Table 6.3 (single-candidate ladder) archived; new `tab:avoiding-projectors` on matched four-candidate cells — MeanFM K3 (seeds 7–10): endpoint 1.000 @ 74.5 ms vs per-step 0.917 @ 148 ms; K3 is off the K1/K2 frontier and dropped; K10 (seed 6): endpoint ahead-or-level and cheaper on CI-MeanFM (1.000 @ 176 vs 0.983 @ 386) and FM (1.000 @ 167 vs 199). Lacking cells → R36.
- **Table 6.5:** K = 1 rows dropped, diffusion K20 last as the sanity reference (R16 → 3 cells). **Table 6.6:** endpoint rows added on the untightened ladder + a conclusion (threshold decides price, not result; endpoint K100 η0.1 stands out); R34 optional. **Table 6.8 + Fig 6.7:** K = 20, η = 0.2 only; diffusion per-step *pending (R2)*, endpoint not defined; CI-MeanFM endpoint *pending (R35)*; text recounted for six comparisons.
- 🔴 **UAV-corridor blanked:** Ch 5/6 corridor passages + 3 figures archived in `withheld/20260922_v3.66_archive/`; new `\outdated{}` marks in Ch 5; Ch 6 keeps the structure with the **new grid** (mf/af 1,2,3; FM 1,2,3,5,20; diffusion 20; PCC always η0.5; HF at K ≥ 3), every cell pending, figures as `\todofigure` specs, result sentences as holes. §6.4 untouched (still quotes v2).
- **Run spec:** `data_status/PENDING_20260922_corridor_v3_run.md` (R33: grid, arms, five Slurm waves, K20 last). Ledger §23; R2/R16 updated; R33–R36 listed. Note to v4.
- `check.py`: all mechanical checks pass; outdated 9, todofigure 3, hole 25. **Bundles (closing v3.66, for the author's full review):** `bundle/output/thesis_v3_20260922_145202_new.zip`, `…_new_clean.zip`, `…_new_nonotes.zip` — 45/45 figures render, cross-references resolve, `--verify` byte-faithful.
