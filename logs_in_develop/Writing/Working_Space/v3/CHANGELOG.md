...
- **Second round (items 13–17):** §6.1.2.3 executed paths → appendix `app:avoiding-paths` (frontier now follows Table 6.2; conclusion names the appendix); **Fig 6.4 withdrawn** (ragged budgets) and replaced by `tab:k-ladder`, the full $\nfe\in\{1,2,5,10,20\}$ grid with eight cells pending **R41** (evaluation only, ~3 h; diffusion K5 optional); §6.2.1.1 paragraph rewritten as prose; Tables 6.7/6.8 print **median and mean** of the final distance (means from the corpus of record); Fig 6.7 → appendix `app:aligning-paths`; ledger R39 (no live cells) + R41. `check.py` passes. **Bundles:** `bundle/output/thesis_v3_20260923_100734_new.zip` + `_clean` + `_nonotes`, 46/46 figures, refs resolve, `--verify` byte-faithful; supersede `204547`.
