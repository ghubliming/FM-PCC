...
On the benchmark of \ac{DPCC} consistency-interpolated average-velocity matching Pareto-dominates --- equal success with constraint satisfaction, fewer control steps and less time per action, the
sense of \autoref{sec:res:avoiding:pareto} --- its
...
control steps. On D3IL-aligning, on ten training-set contexts, the dominance repeats where the task separates the models: without projection, at
twenty sampling steps, analytic average-velocity matching moves the box in
every context and reduces the median final distance by $84\,\%$ of the mean initial distance,
...
constraint, keeping every context violation-free, one of them held by the guard, at the price of per-step projection.

The quadrotor scenes carry the framework to a plant that needs feedback even to hover. UAV-pillars flies
...
demonstration satisfies, and that on straight-line demonstrations the objectives cannot be told
apart and the budget decides; there the two projectors trade against each other without one dominating.
...
\begin{description}
  \item[RQ1 --- Generative model.] Yes, with a margin, on the benchmark of \ac{DPCC}. With the
    backbone, the data and the evaluation held fixed, and each objective trained as \autoref{tab:train} states
    it, consistency-interpolated average-velocity
    ...
    trained and run at one denoising step loses it (\autoref{tab:avoiding-conclusion}). On D3IL-avoiding and
    ...
    and differ in cost. On D3IL-aligning without projection, on ten training-set contexts, analytic average-velocity matching
    ...
    flights of ten, the consistency-interpolated model on six and the analytic one on three. One evaluation saturates D3IL-avoiding, D3IL-aligning is run at its
  ...
  \item[RQ2 --- Constraints.] Endpoint projection guides the sample only where the budget and the
    ...
    average-velocity matching under random selection all ten (one held by the guard) against nine at $275$ against
