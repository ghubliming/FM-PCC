...
constraint tightening \parencite{romer2025diffusion}; its receding-horizon loop is the one this thesis
inherits (\autoref{sec:method:dpcc}). It is both the codebase this thesis starts from and the baseline it
is measured against.
...
instead learn a few-step generator directly.
...
\parencite{kondo2024cgd}. Below the planner, geometric tracking control on the special Euclidean group tracks position and attitude
without the singularities of local coordinates \parencite[Eqs.~(19)--(23), Prop.~3]{lee2010geometric},
with the allocation from a commanded force to the rotor thrusts of \parencite{mellinger2011minimum}; these
are the equations the controller of \autoref{sec:method:geometric} implements. Predictive sampling offers derivative-free model
