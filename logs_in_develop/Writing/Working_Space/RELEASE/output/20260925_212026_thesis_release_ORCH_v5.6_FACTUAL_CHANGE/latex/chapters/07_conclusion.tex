...
On the benchmark of \ac{DPCC} consistency-interpolated average-velocity matching Pareto-dominates --- ahead on every reported axis at once, the sense of
\autoref{sec:res:avoiding:pareto} --- its
...
\begin{description}
  \item[RQ1 --- Generative model.] Yes, with a margin, on the benchmark of \ac{DPCC}. With the
    ...
    run away from its trained budget, loses it (\autoref{tab:avoiding-conclusion}). On D3IL-avoiding and
