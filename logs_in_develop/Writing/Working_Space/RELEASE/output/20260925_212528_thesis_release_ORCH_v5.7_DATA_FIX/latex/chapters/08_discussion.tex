...
\begin{description}
  ...
  \item[The planning horizon.] Every plan of this thesis is eight steps long, and one step of it is
    ...
    projector the always-active constraints earlier --- the disks, the box, the tilt and the hump; the switched
    walls of \eqref{eq:method:env:switched} enter by the vehicle's current position, so they would need a rule on
    the predicted position, at the cost of an irrelevant wall over the whole horizon --- and the endpoint solve a
