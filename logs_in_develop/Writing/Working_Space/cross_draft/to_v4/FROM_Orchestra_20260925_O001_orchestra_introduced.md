# FROM Orchestra → v4 · 2026-09-25 · O001 · orchestra_introduced
...
**Nothing is asked of v4 now; nothing of yours was changed.** This note introduces a new chat and its protocol, so that a change you did not make is never a surprise.

## What the Orchestra does (author, 2026-09-25)

The author runs a separate **Orchestra** chat (`Working_Space/Orchestra/`, runbook in its `README.md`) for **minor changes, and changes that are cross-linked between chapters of different drafts**. It edits directly inside `v4/` — **only the files you own** per `DRAFT_OWNERSHIP.md`, never an inherited copy, never your README / state files / notes — and records every edit in **your `CHANGELOG.md`** as a revision entry, newest first, in your format. Big changes stay with you; the author brings them to your chat directly. When a heavy job is split up, you receive your part as a **TODO list** through this inbox.

## How an Orchestra edit shows up in your files

1. **A revision entry in your `CHANGELOG.md`** — **v4: the next letter** of the current revision (v4.2 → v4.2a), the pattern you use for small passes; the heading is tagged `(Orchestra O###)` and links the job file. Your next pass continues as usual (v4.3 or the next letter). The body names the file and lines, the author's request, the checks run (**never compiled**), and what is left to you.
2. **A cross note + INBOX row** (`cross_draft/to_v4/FROM_Orchestra_<date>_O###_<topic>.md`, row `Orchestra O### · <date>`) for every job that touched your files, and an FYI note when a change elsewhere affects something you reference (a label, a term, a restated number).
