...
| **`Naming/`** | 📁 🟢 **THE naming folder — start at [`Naming/TRANSLATION_20260914_dev_jargon_to_scientific.md`](Naming/TRANSLATION_20260914_dev_jargon_to_scientific.md).** Three columns: *code token* │ *paper name* │ *what it really is → thesis name*, for every engine, arm, rule, backbone, geometry knob and embodiment, plus the five standing rules. **Canonical — where it and any note below disagree, it wins.** |
...
| **`NOTES_plotting_sources.md`** | 🖼️ **Hint: figures are not limited to the curated report plots.** The DA pipelines (`DA_Code_v3`, `DA_VA_v2`, `DA_UAV_v1`) and the HTML visualizers in `Data_Analysis/` can be referred to and their code reused — including the visualizer's PNG + LaTeX result-matrix export. Notes the cluster-only dependencies and the provenance check. |
| **`NOTES_plan_quality_and_smoothness.md`** | 📉 **Why downstream metrics hide the generator, and the one measurement that is still owed.** Every reported number sits behind a projector *and* a tracking controller, both low-pass: at one network evaluation the diffusion baseline's raw plans are a scribble whose *commanded* traces look smooth only because the controller integrates them away. Unprojected, MeanFlow has **4.7× fewer violations at matched compute**. Carries the numbers, the ladder warning for the consistency target, and the missing jerk/curvature measurement (**no new runs needed**). |
