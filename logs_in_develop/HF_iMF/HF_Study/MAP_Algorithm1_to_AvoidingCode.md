...
Algorithm 1 is written for a **general differentiable affine scheduler** $(\alpha_t, \beta_t)$ with
$X_t = \alpha_t X_1 + \beta_t X_0$. The code only ever uses **one specific scheduler**: the standard
conditional flow-matching (CFM) interpolant.

| | Paper (general) | Code (CFM, hard-coded) |
|---|---|---|
| Interpolant | $X_t = \alpha_t X_1 + \beta_t X_0$ | `flow_matcher.py:71` — `ConditionalFlowMatcher.compute_mu_t`: `t*x1 + (1-t)*x0` |
| ⇒ | $\alpha_t = t$, $\beta_t = 1-t$ | (read off directly) |
| $\dot{\alpha}_t$, $\dot{\beta}_t$ | scheduler derivatives | `1`, `-1` |
| $\Lambda_t := \alpha_t \dot{\beta}_t - \dot{\alpha}_t \beta_t$ | (Lemma, `main.tex:373`) | $t(-1) - 1(1-t) = -1$ (**constant**, never appears explicitly in code — it's already been divided out) |
| Training target $v_{t\mid Z} = \dot{\alpha}_t X_1 + \dot{\beta}_t X_0$ | (`main.tex:369`) | `flow_matcher.py:85` — `compute_conditional_flow`: `x1 - x0` (matches: $1\cdot X_1 + (-1)\cdot X_0$) |

Every $\Lambda_t$ in the paper's formulas below has already been replaced by `-1` in the code —
...
|---|---|---|---|
| $v_t^\theta(x)$ | learned marginal velocity field | `self.flow_model(x, t)` (called via `constrained_flow_fn_torch`) | `flow_policy.py:1709`, `1310-1314` |
| `x` (generic point in $\mathbb{R}^d$) | — | for this task, `x` is a **flattened (action,state) trajectory**, not a point — see §4 | — |
| $x_i$, $\bar{x}_{i+1}$, $x_{i+1}$ | current / nominal-next / corrected-next ODE state | `x_k`, `x_next_ref`, `x_next` | `flow_policy.py:1323,1325,1360` |
| $t_i$, $\Delta t_i$ | time grid, step size | `t_k`, `dt` | `flow_policy.py:1322,1320` |
| $N$ (discretization steps) | | `self.oc_N_steps` = `cfg.ode_t_steps` (default **20**) | `flow_policy.py:700`; `config/flow_matching.py:65` |
| $\mathcal{M}_t^\theta(x) := \mathbb{E}[X_1 \mid X_t=x]$ (posterior mean, Lemma `main.tex:361-380`) | | `x_terminal_predicted_ref` (the *unconstrained* version, called $\bar{x}_N$ below) | `flow_policy.py:1340` |
| $\bar{x}_N := \mathcal{M}_{t_{i+1}}^\theta(\bar{x}_{i+1})$ | nominal (unconstrained) predicted terminal state | `x_terminal_predicted_ref` | `flow_policy.py:1340` |
...
| $h(x) \le 0$ | hard constraint | pillar/boundary/quadrilateral inequalities | `flow_policy.py:280-348` (`_apply_obstacle_constraints`) — see §5 |
| $C(x)$ | optional cost/reward | `-self.oc_distance_objective` | `flow_policy.py:734-743` (`objective="distance"` branch) |
| $\lambda_{oc}$ (control-regularization weight) | | `self.cfg.hardflow_reg_scale` (default **1.0**) | `flow_policy.py:712`; `config/flow_matching.py:85` |
| $\alpha_{t_{i+1}}$ (in the cost weight $\frac{\lambda_{oc}}{2\Delta t_i} \alpha_{t_{i+1}}^2$) | | `self.oc_t_param` = `t_k+dt` = $t_{i+1}$ (since `α_t=t` under CFM) | `flow_policy.py:714,1342` |
...
### Line: `\KwIn{...}`
Inputs: `p_0`, `v_t^θ`, `C(·)`, `h(·)≤0`, $\lambda_{oc}$, `N`, time grid, scheduler `(α_t,β_t)`.
...
### Line: `Draw initial state` $\bar{x}_0 \sim p_0$ `and set` $x_0=\bar{x}_0$
...
### Line: `Compute` $\Delta t_i = t_{i+1} - t_i$

Code (`flow_policy.py:1320,1322`): `dt = 1.0 / self.oc_N_steps`; `t_k = k * dt` — uniform grid,
so $\Delta t_i$ is the same constant every step (not recomputed per-step, just baked into `dt`).

### Line: `Compute` $\bar{x}_{i+1} = x_i + v_{t_i}^\theta(x_i) \Delta t_i$
...
### Line: `Compute` $\bar{x}_N = \frac{\dot{\beta}_{t_{i+1}} \bar{x}_{i+1} - \beta_{t_{i+1}} v_{t_{i+1}}^\theta(\bar{x}_{i+1})}{\Lambda_{t_{i+1}}}$

This is $\mathcal{M}_{t_{i+1}}^\theta(\bar{x}_{i+1})$ — the posterior-mean/x-prediction formula from Lemma 1
(`main.tex:361-380`), evaluated at the nominal next state.

Substituting the CFM values from §1 ($\dot{\beta}=-1, \beta=1-t, \Lambda=-1$):
$$
\begin{aligned}
...
- cost (`flow_policy.py:710-715`):
  ...
  self.oc_control_cost = 0.5 * self.cfg.hardflow_reg_scale \
      ...
      * self.oc_t_param**2
  ```
  `hardflow_reg_scale` plays the role of $\lambda_{oc}/\Delta t_i$ (the $1/\Delta t_i$ is pre-folded into the single
  config constant since the grid is uniform — $\Delta t_i$ never changes, so there's no reason to
  divide by it every step); `oc_t_param**2` is `α_{t_{i+1}}²` ($=t_{i+1}^2$ under CFM).
  ...
  (`objective=""`).
- constraint $h(\hat{x}_N) \le 0$ (`flow_policy.py:717-728`, dispatches to `_apply_obstacle_constraints`
...
### Line: `Compute` $x_{i+1} = \alpha_{t_{i+1}} \hat{x}_N^* + \beta_{t_{i+1}} \frac{-\dot{\alpha}_{t_{i+1}} \bar{x}_{i+1} + \alpha_{t_{i+1}} v_{t_{i+1}}^\theta(\bar{x}_{i+1})}{\Lambda_{t_{i+1}}}$

This is the line that looks **nothing like the code** until you expand it. It's
$\alpha_{t_{i+1}} \hat{x}_N^* + \beta_{t_{i+1}} \mathcal{W}_{t_{i+1}}^\theta(\bar{x}_{i+1})$ where $\mathcal{W}$ is the
*noise*-side posterior estimate (the other half of Lemma 1, `main.tex:377-379`).

Substitute CFM values ($\alpha=t, \beta=1-t, \dot{\alpha}=1, \Lambda=-1$):
$$
\begin{aligned}
...
\end{aligned}
$$
**Identical to (\*).** ✓ The code's one-liner $x_{next\_ref} + t_{i+1}(\hat{x}_N^* - \bar{x}_N)$ (a "gain-$t_{i+1}$
...
$x = \alpha_t \mathcal{M}_t(x) + \beta_t \mathcal{W}_t(x)$ (`main.tex:389`, "posterior_identity") before being coded up.
...
Algorithm 1's $x \in \mathbb{R}^d$ is written for a generic vector. On this task, `x` is a **flattened
trajectory**: `H` timesteps of `(action, state)` pairs, i.e.
$x = [a_0, s_1, a_1, s_2, \dots, a_{H-1}, s_H]$ — `H·(action_dim+state_dim)` numbers, **minus**
...
argument the paper's abstract $v_t^\theta(x)$ doesn't need — the network always sees the full
trajectory including $s_0$, but the *optimizer* never touches $s_0$, only the free `oc_dof`
coordinates.

---

## 5. What $h(x) \le 0$ actually *is* (the avoiding task's obstacles)
...
- **circular keep-out** around each pillar (`flow_policy.py:315-322`):
  $\|pos - pillar\_center\| \ge radius + margin \iff h = radius+margin-\|pos-center\| \le 0$
...
(state, action) pair — this has nothing to do with Algorithm 1's $h(x_N) \le 0$ (that's a
...
| `_apply_dynamics_constraints` | `flow_policy.py:350-441` | Extra physical-plausibility equality constraints, orthogonal to the paper's single terminal inequality $h(x_N) \le 0$. |
...
| $\lambda_{oc}$ (× the folded $1/\Delta t_i$) | `hardflow_reg_scale` | `1.0` | `config/flow_matching.py:85` |
| cost-term scale (if $C \ne 0$) | `hardflow_cost_scale` | `1.0` | `config/flow_matching.py:86` |
...
$h(x_N) \le 0$, provided the feasible set is nonempty."* — Why this is true in the code: at the
**last** iteration (`k = N-1`, so `t_k+dt = 1`), `x_terminal_predicted_ref` (`flow_policy.py:1340`)
reduces to $x_{next\_ref} + 0\cdot v_{next} = x_{next\_ref}$ (since $1-t_{i+1}=1-1=0$), and the final update
(`flow_policy.py:1360-1362`) becomes $x_{next} = x_{next\_ref} + 1\cdot (x_{terminal\_predicted} - x_{terminal\_predicted\_ref}) = x_{terminal\_predicted} = \hat{x}_N^*$ — i.e. **the last output *is* the constrained NLP solution**,
which by construction satisfies `h(·)≤0`. This is the algebraic content of the paper's
$\mathcal{M}_{t_N}^\theta(x_N)=x_N$ "posterior estimator becomes exact unconditionally at the final step"
