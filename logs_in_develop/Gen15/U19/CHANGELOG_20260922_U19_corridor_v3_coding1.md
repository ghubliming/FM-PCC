...
| G4 | to submit (author: run as is, no env change) | to submit | **paper driver `Slurm_Codes/temp_bash/eval_20260923_p23_corridor_v3.sh`** (23-09 runbook §1–§2: paper-only variants, tightened, PCC r/c/t + HF single/r/c/t, K ladders incl. 2 and fm 20; waves C1–C5; tags `p23cv3t` / `p23cv3ah`; 68 cells per scene) — started wave-by-wave through **`…_master.sh start`** (one 10-min CPU link per wave, submits its wave with the ids recorded via `P23_DIRECT=1`, queues the next link with `--dependency=afterany`; squeue holds one wave + one pending master). The 22-09 driver's `submit` (U16 layout, `u19cv3*`) is superseded. DA with `DA_UAV_v1`, tag-only |
