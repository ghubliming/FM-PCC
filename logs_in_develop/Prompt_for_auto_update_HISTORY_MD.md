...
| Field | Value |
|---|---|
| **Commit hash / Date** | `b8f1beb2` / `2026-09-18` |
| **Commit message / Scope** | `Gen15U17 new UAV pillars Constraints (for Paper).; Docs Update` |
| **Commit date** | 2026-09-18 22:21:50 UTC |
| **What was recorded** | Entries through September 18, 2026:<br>1. **UAV Pillars Passive Feasibility Diagnosis & Active Avoidance Redesign** (`CHANGELOG_20260918_pillars_enlarged.md`, `PENDING_20260918_pillars_geometry_redesign.md`): Diagnosed that expert demonstrations already sat outside `pillars_hg` keep-out zones ($|y|=1.11$ vs $1.03$ m), causing projection to perturb clean plans rather than solve infeasibility; established test-time active avoidance requirement.<br>2. **Gen15 U17 Enlarged Obstacle Geometry (`pillars_xl`, `pillars_xxl`)** (`config/uav_projection.yaml`, commit `b8f1beb2`): Added virtual enlarged pillars ($R=0.35$ m, keep-out $0.66$ m) forcing a $0.15$ m trajectory detour while leaving a solvable $0.93$ m corridor; verified dynamic execution scorer integration; configured disjoint K=5 evaluation waves (`eval_20260919_u17_pillars_xl.sh`).<br>3. **Trajectory Path Extraction & Expert Data Visualizations** (`expert_paths.py`, `uav_paths.py`, `builders/expert.py`, `builders/paths.py`): Built extraction and SVG/PNG rendering pipelines for executed drone paths (`corridor`, `pillars`, `s_curve`) and training demonstrations (`fig_expert_aligning`, `fig_expert_uav`); created `pillars_grid.py` for automated evaluation analysis. |
