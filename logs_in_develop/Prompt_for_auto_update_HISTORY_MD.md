...
| Field | Value |
|---|---|
| **Commit hash / Date** | `105f1968` / `2026-09-17` |
| **Commit message / Scope** | `Doc Update 17-Sep-III; SLURM runs for Paper DA ready` |
| **Commit date** | 2026-09-17 22:03:21 UTC |
| **What was recorded** | Entries through September 17, 2026:<br>1. **D3IL-Avoiding DPCC Protocol Evaluation Tools & MeanFlow U-Net Discovery** (`avoiding_rules_by_protocol.py`, `avoiding_seed_spread.py`, `REQUEST_20260917_avoiding_dpcc_protocol.md`): Built DPCC-protocol evaluation tools; recovered existing MeanFlow U-Net K=1/2 5-seed rows in the 15-09 batch; scheduled missing FM and α-Flow evaluation runs.<br>2. **Automated UAV & VA Result Aggregation** (`uav_results.py`, `va_results.py`): Enhanced automated result extractors for multi-axis metrics across controllers, environments, and model architectures.<br>3. **UAV Constraints Visualization & Trajectory Overlay Request** (`fig_constraints_uav`, `REQUEST_20260917_trajectory_figures.md`): Rendered top-down 2D constraint geometry and specified remote rollout artifact export for executed flight path figures.<br>4. **Cluster Script Housekeeping** (`Slurm_Codes/temp_bash/`): Purged obsolete temporary test scripts to maintain cluster entrypoint hygiene. |
