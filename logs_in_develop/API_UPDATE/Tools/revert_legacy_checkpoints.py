#!/usr/bin/env python3
"""
revert_legacy_checkpoints.py — Revert one FM-PCC checkpoint folder after a mistaken API rename.

Scan-then-fix flow:
  1. SCAN  — check folder name, every .pkl, every args.json for patched tokens.
  2. REPORT — print exactly what is patched and what is already clean (legacy).
  3. FIX   — patch only the items that are actually patched; leave clean items untouched.

Usage:
    python revert_legacy_checkpoints.py --path <folder> [--dry-run] [--backup]
"""
...
# Reversed from patch_legacy_checkpoints.py
TOKEN_REMAP = [
    ...
    ('models.imf_diffusion.iMeanFlowODE', 'models.imf_diffusion.iMFDiffusion'),
...
def main():
    ...
    if not os.path.isdir(folder):
        print(f'[ revert ] ERROR: path does not exist or is not a directory.')
        sys.exit(1)

    print(f'[ revert ] Path    : {folder}')
    print(f'[ revert ] Dry-run : {args.dry_run}')
    print()
    ...
    print('[ scan ] Folder name  :', 'REVERT REQUIRED' if legacy_folder else 'clean')
    for fpath in sorted(legacy_pkls):
        print(f'[ scan ] pkl   REVERT  : {os.path.relpath(fpath, folder)}')
    for fpath in sorted(legacy_jsons):
        print(f'[ scan ] json  REVERT  : {os.path.relpath(fpath, folder)}')
    for rel, err in sorted(errors.items()):
        print(f'[ scan ] pkl   ERROR   : {rel}  ({err})')

    if not any_legacy and not errors:
        print()
        print('[ revert ] Everything is already original — nothing to do.')
        sys.exit(0)
    ...
    if legacy_folder:
        ...
        if os.path.exists(new_folder):
            print(f'  [dir]  WARNING: revert target already exists — skipping.')
            print(f'         {new_folder}')
        else:
    ...
    print()
    print(f'[ revert ] {"(dry) " if args.dry_run else ""}{n_fixed} item(s) reverted.')
