...
### B. "Desired" vs. "Real" Trajectories
To maintain parity with the FMv3ODE diagnostics, we track two distinct paths during evaluation:
1.  **Desired Trajectory (The Plan):** The raw, 8-step future plan generated by the U-Net at any given tick. This represents the model's theoretical "best path" based on its visual perception.
2.  **Real Trajectory (The Execution):** The actual (x,y,z) coordinates achieved by the robot in MuJoCo. 

**Diagnostic Value:** Comparing these two paths allows us to measure the **Tracking Error**. In Gen5, high tracking errors suggest that the visual model is planning paths that are physically unrealistic or that the simulator's control loop is failing to keep up with the U-Net's speed.
