# Visual-Mix-ML (Gen14) evaluation script.
# Copy-modified from fm_visual_aligning_test/eval_fm_visual_aligning.py (Gen7 @ HEAD), which
# carries the C4 grouped-JSON schema, the C5 consolidated NPZ, the C6 projection circuit
# breaker, the D1 box-obstacle guard and the U19 foresight plot. Everything except the two
...
#
# The env, episode budget, MPC candidate pool, DPCC projector and constraint YAML are
# IDENTICAL across arms — only the generative engine differs. That is what makes the
# four-way comparison meaningful.
#
# Usage:  python -m mix_visual_aligning_test.eval_mix_visual_aligning --engine mf --seed 6
#
# Output:  logs/aligning-d3il-visual/plans/mix_visual_aligning_<engine>/<exp>/results/<seed>/
...
def setup_dpcc_projector(args, config, obs_normalizer, act_normalizer, variant,
    ...
    if hasattr(obs_normalizer, 'mins') and len(obs_normalizer.mins) > _target_obs_dim:
        from mix_visual_aligning.datasets.normalization import LimitsNormalizer as _LN
...
if __name__ == '__main__':
    ...
    for _seed_i, seed in enumerate(seeds):
        ...
        # The plan block carries its own 'engine' key; a disagreement with --engine means
        # the wrong config block is being read for this arm.
        ...
        if not args_cli.aggregate_only:
            ...
            # Set clip_denoised=True in plan_mix_visual_aligning config only to ablate.
            ...
            if ENGINE == 'diffusion':
                ...
                print(f'[ eval ] flow_steps_v3 = {_flow_steps}  '
            ...
            if ENGINE_SPEC['two_time']:
                print(f'[ eval ] two-time engine: u-head only at sampling, '
                      f'h-conditioned Euler (v head dropped by design)')
