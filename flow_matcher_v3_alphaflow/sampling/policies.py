...
class Policy:
    ...
    def __call__(self, conditions, batch_size=1, horizon=16, test_ret=None, constraints=None, disable_projection=False):
        ...
        # fix_4 (mirror of Gen3v6 fix_5, ecbae16f): `which_trajectory` always indexes the
        # ORIGINAL (unsorted) batch, because `actions` is never reordered (see the
        # `actions[which_trajectory, 0]` below). The temporal-consistency branch DOES
        # reorder `observations`, so indexing it with `which_trajectory` applies `order`
        # twice and picks a different candidate than the one being executed.
        # `executed_idx` is the index of the executed plan *within `observations` as it
        # stands at the assignment* — 0 after the sort, `which_trajectory` otherwise. This
        # restores the invariant MPC_NPZ_PATCH (JOB C) intended: prev_observations is the
        ...
        if self.trajectory_selection == 'temporal_consistency' and not disable_projection and self.prev_observations is not None:   # Temporal consistency
            ...
            which_trajectory = 0
            executed_idx = 0
        self.prev_observations = np.repeat(np.expand_dims(observations[executed_idx], axis=0), batch_size, axis=0)  # MPC_NPZ_PATCH + fix_4
