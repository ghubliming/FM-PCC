"""Gen3v7 U3 — HardFlow's `hardflow_new` constrained sampler, on the α-Flow field.
...
(`flow_matcher_v3_meanflow/sampling/hardflow_projection.py`) -> here. The NLP, the dof
layout, the prox schedule and the pull-back are BYTE-IDENTICAL to Gen3v6's post-fix_4
copy. Exactly three things are generation-specific and were re-derived from Gen3v7's own
engine rather than copied:
  1. `init_noise_scale` -> α-Flow's sampler is sigma=1.0 (`af_diffusion.py:260`).
  2. the h=0 grounding argument (below) -> α-Flow TRAINS that anchor directly.
  3. every docstring/gate reference now points at Gen3v7 files.
See `logs_in_develop/Gen3v7_AlphaFlow/init/PORT_GUIDE_hardflow_into_Gen3v7.md`.
