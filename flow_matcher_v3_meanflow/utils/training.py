...
# ── Gen3v6 — extra per-step metrics from MeanFlowODE._build_info ───────────────────────
...
# (PLAN §3.4) — the metric COMPARE §7.4.1 asked for a month ago and no generation had.
...
class Trainer(object):
    def __init__(
        ...
        # 🔴 Gen3v6 — `gradient_clip` was a DEAD config key in this lineage: it existed in
        ...
        # the v-head loss on both sides (their loss_v analog). In Gen3v6 these are aliases of
        ...
        # Gen3v6: h-stratified + raw-MSE metric family (see EXTRA_METRIC_KEYS)
        self.train_extra_losses = {k: [] for k in EXTRA_METRIC_KEYS}
        self.test_extra_losses = {k: [] for k in EXTRA_METRIC_KEYS}
        # Gen3v6: observed grad-norm BEFORE clipping — tells you whether gradient_clip is
    ...
    def train_epoch(self, n_train_steps, epoch=0):        
        ...
        for step in range(n_train_steps):
            ...
            # Gen3v6: gradient clipping is APPLIED here (it was a dead key upstream).
    ...
    def test(self, n_test=100):
        ...
        test_aux = 0
        aux_seen = False
        # Gen3v6: NaN-safe running means for the extra metric family (empty h-buckets)
    ...
    def _checkpoint_payload(self):
        ...
        data = {
            ...
            'grad_norm_history': self.grad_norm_history,   # Gen3v6
        }
        for key in EXTRA_METRIC_KEYS:                      # Gen3v6
    ...
    def save_losses(self):
        ...
        # Gen3v6 extra metric family (h-stratified residuals, raw MSEs, sampler sanity)
        for key in EXTRA_METRIC_KEYS:
    ...
    def load(self, epoch):
        ...
        if 'train_losses' in data:
            ...
            self.grad_norm_history = data.get('grad_norm_history', [])        # Gen3v6
            for key in EXTRA_METRIC_KEYS:                                     # Gen3v6
            ...
            if os.path.exists(losses_path):
                try:
                    ...
                    self.grad_norm_history = losses.get('grad_norm_history', [])           # Gen3v6
                    for key in EXTRA_METRIC_KEYS:                                          # Gen3v6
