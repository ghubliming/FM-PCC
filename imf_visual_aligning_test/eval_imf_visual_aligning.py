# Visual-iMF (Gen8) evaluation script.
# Copy-modified from fm_visual_aligning_test/eval_fm_visual_aligning.py (Gen7).
# Logging pattern (realtime PNG/JSON/pkl, 7-metric report, expert reference,
# legacy rollout grid) is reused verbatim from Gen6V4/Gen7.
#
# Key differences from Gen7 eval:
#   - Uses imf_visual_aligning (iMeanFlow ODE engine replacing vanilla FM ODE)
#   - experiment='plan_imf_visual_aligning' (new config block)
#   - iMF dual-head u/v inference; u-only at sampling (Deviation A fix inherited)
#   - h-conditioning active: VisualUNet.backbone has h_mlp
#
# Output:  logs/aligning-d3il-visual/plan_imf_visual_aligning/<exp>/results/<seed>/
...
def setup_dpcc_projector(args, config, obs_normalizer, act_normalizer, variant,
    ...
    if hasattr(obs_normalizer, 'mins') and len(obs_normalizer.mins) > _target_obs_dim:
        from imf_visual_aligning.datasets.normalization import LimitsNormalizer as _LN
...
def _rebuild_engine_config_from_path(lp, device='cuda:0'):
    ...
    # parse H{horizon}
    m_h = _re.search(r'_H(\d+)_|^H(\d+)_', exp_dir)
    horizon = int((m_h.group(1) or m_h.group(2))) if m_h else 8
    # parse V{True|False}
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        args = Parser().parse_args(experiment='plan_imf_visual_aligning', seed=seed)
