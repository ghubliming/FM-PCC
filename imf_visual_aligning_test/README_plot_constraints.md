...
It imports and utilizes the `plot_geo_constraints` function from `eval_fm_visual_aligning.py` to generate a 3-panel constraint overview figure (3D wireframe, XY top-down, and XZ side) for each configuration without needing to run the full D3IL evaluation pipeline.
...
### How to Use
Navigate to the `fm_visual_aligning_test` directory and run the script:

```bash
cd /workspaces/FM-PCC/fm_visual_aligning_test
