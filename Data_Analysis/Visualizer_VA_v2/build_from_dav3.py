...
sub('<div>SCIENTIFIC_SUITE_v3.15</div>',
...
# the LaTeX preamble note still pointed at the old table numbers
sub("""           r"In Tables~3 and~4 a trailing {\\tiny (goal, constraint)} marks a cell whose \"""",
    """           r"In the MIN\\_DIST, N\\_STEPS and AVG\\_TIME tables a trailing {\\tiny (goal, constraint)} marks a cell whose \"""")

# ── 12c. U3: the INIT XY reference row + the per-candidate run tally ─────────
# REF_METRIC joins the fetched metrics but must NOT invent table columns/rows of
# its own, hence the `core` frame for the cands/variants axes.
sub("""    metrics = [t[0] for t in SUMMARY_TABLES]
    if seed_mode == "custom":""",
    """    metrics = [t[0] for t in SUMMARY_TABLES] + [REF_METRIC]
    if seed_mode == "custom":""")
sub("""    cands = sorted(d['Candidate'].astype(str).unique(), key=lambda c: (0, int(c)) if c.isdigit() else (1, c))
...
sub("""def _render_matrix(rec, metric, fmt, annotate, cands, variants):""",
...
sub("""        rows.append("<tr>" + "".join(cells) + "</tr>")
...
sub("""        lines.append(_tex_escape("CAND_" + str(c)) + " & " + " & ".join(cells) + r" \\\\")
...
sub("""              'Rows follow <b>6. Candidates</b>; columns are always every variant. '
              f'<b>EXPORT ZIP</b> writes exactly these {len(SUMMARY_TABLES)} tables to a .tex file inside the archive.'
              '</div>']
...
# CSS for the two new pieces (anchored on this page's own block from step 2)
sub("""        .view-toggle { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 4px; }""",
    """        .view-toggle { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 4px; }
