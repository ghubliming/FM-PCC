...
# ⚠️ MATCHED BUDGET OR NOTHING (PLAN §8). Mirrors the eval script's --flow-steps so the
# aggregation reads the SAME K that was evaluated; results live in a per-K directory
# (flow_steps_v3 is watched as 'K'), so without this the K=2 default would be reported no
# matter which budget was actually run.
_ap = argparse.ArgumentParser(description='Aggregate α-Flow (Gen3v7) eval results.')
...
if _cli.flow_steps is not None:
    _blk = importlib.import_module('config.' + exp).base['plan_fm_v3_alphaflow']
    _blk['flow_steps_v3'] = _cli.flow_steps
    if 'ode_inference_steps_v3' in _blk:
        _blk['ode_inference_steps_v3'] = _cli.flow_steps
    print(f'[ load_results ] Overriding flow_steps_v3 (K) to: {_cli.flow_steps}')
