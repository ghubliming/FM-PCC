# Gen3v7 α-Flow evaluation — DPCC-projected planning on D3IL avoiding.
# ✅ α is TRAINING-ONLY: it does not appear at eval and the sampler is unchanged (x += dt·u).
# Nothing about inference differs from Gen3v4/Gen3v6, which is exactly what makes the
# three-way matched-K comparison clean (PLAN §5.3).
# ⚠️ MATCHED BUDGET OR NOTHING (PLAN §8): sweep flow_steps_v3 ∈ {1,2,5,10} and compare only
# at equal K. fix_7.3 §9 — one hard-coded k_steps=10 killed an entire generation's claim.
# Sibling of FM_v3_meanflow_test/eval_flow_matching_v3_meanflow.py; loads the
# 'plan_fm_v3_alphaflow' config block. See
# logs_in_develop/Gen3v7_AlphaFlow/init/PLAN_Gen3v7_alphaflow.md.
import time
import yaml
import os
import torch
...
# ⚠️ MATCHED BUDGET OR NOTHING (PLAN §8; fix_7.3 §9 — one hard-coded k_steps=10 made the
# decisive control unrunnable and killed an entire generation's claim). K is a first-class
# CLI knob here so the {1,2,5,10} grid is a loop in the sbatch, not a config edit someone
# has to remember. `flow_steps_v3` is in args_to_watch_fmv3_ode_plan (label 'K'), so each K
# writes its OWN results directory and no two budgets can overwrite each other.
...
# Gen3v7 U3: repointed from the SHARED config/projection_eval.yaml to the Gen3v7-dedicated
# unified file (DPCC arms + HardFlow arm in one document). The shared file is read by ~50
# files across every generation, so adding 'hardflow_new' there would inject arm C into
# generations that have no HardFlowPolicy. --config overrides it.
_default_cfg = 'config/alphaflow_projection_eval.yaml'
...
# 🔴 PATH COLLISION GUARD — the arms-A/B fan is NOT a results-folder token, so an mpc4 and an
# mpc1 run at the same K/A/T would write to the SAME directory and clobber each other.
# Promoting it to a real token would rename every historic path, so a non-default fan auto-tags
# itself via the existing custom-message slot instead (the guard Gen3v6 U10 uses for the replan
# cadence). An explicit FMPCC_RUN_MSG always wins. Must be set BEFORE the first
# Parser().parse_args(), which is what imports config/<exp>.py.
...
if args_cli.flow_steps is not None:
    # Patch the config MODULE's dict before any Parser reads it. utils.Parser.read_config
    # does `importlib.import_module(args.config)` and copies `base[experiment]` key by key,
    # and Python caches modules — so this is the intended data path, not a monkey-patch:
    # exp_name, savepath and the diffusion kwargs all follow automatically.
    import importlib
    for _exp in exps:
        _mod = importlib.import_module('config.' + _exp)
        _blk = _mod.base['plan_fm_v3_alphaflow']
        _blk['flow_steps_v3'] = args_cli.flow_steps
        if 'ode_inference_steps_v3' in _blk:
            _blk['ode_inference_steps_v3'] = args_cli.flow_steps
        # Gen3v7 U3 — matched-K: `flow_steps` is arm C's Euler K. Patch it with the SAME value
...
for exp in exps:
    for halfspace_variant in halfspace_variants:
        ...
        ran_variant_idx = set()
        for seed in seeds:
            args = Parser().parse_args(experiment='plan_fm_v3_alphaflow', seed=seed)
            ...
            if not args_cli.aggregate_only:
                ...
                def load_diffusion_with_override(*loadpath, target_class=None, epoch='best', device='cuda:0', seed=None):
                    ...
                    _SAMPLING_OVERRIDE_KEYS = {
                        ...
                        # NOTE (Gen3v7): the meanflow_cfg_* keys are gone — this generation
                ...
                # eval_use_ema config switch. Gen3v7 DEFAULTS TO TRUE: few-step MeanFlow-family is
                ...
                # Gen3v7 U3 — matched-K. K arrives via the config (args.flow_steps / flow_steps_v3,
                # both reading HFFM_FLOW_STEPS or --flow-steps), so args.savepath already encodes
                # _K{K}_ (distinct-K runs never collide). Force it onto the native sampler too —
                # the getattr above can otherwise pick up a stale checkpoint ode_inference_steps_v3
                # instead of this K, which would leave arms A/B and arm C on different budgets.
                ...
                if fm_model.__class__.__name__ in ['FlowMatchingIMF', 'AlphaFlowODE']:
            ...
            for variant_idx, variant in enumerate(projection_variants):
                ...
                try:
                    ...
                    if 'dt0p25' in variant:
                        ...
                        delta_t = 4.0 * dt
                    if is_hardflow:
                        # ---------------- arm C (HardFlow) — verbatim Gen12/Gen3v6 construction ----------------
                        ...
                        policy = HardFlowPolicy(
                            ...
                            # fix_4 (Gen3v6): arm C must start from the SAME noise law as arms A/B.
                            # α-Flow's sampler is sigma=1.0 (af_diffusion.py:260). 🔴 NOT the 0.5 in
                            # flow_matcher_v3_alphaflow/models/diffusion.py — that is the legacy
                            # FMv3ODE class sharing the folder. Stated explicitly here so a
                            # generation swap can't reintroduce the Gen3v6 fix_4 bug silently.
                    ...
                    # Gen3v7 U3 — HardFlow-arm metrics (0 for arms A/B).
                    nfe_total = 0
                    ...
                    for i in range(n_trials):
                        ...
                        rt_rec = RTRecorder(episode_id=f'{exp}_{variant}_seed{seed}_trial{i}',
                                            ...
                                            system='FMv3_AlphaFlow',
                                            control_hz=RT_CONTROL_HZ,
                                            # Gen3v7 U3: the ARM's fan size (HFFM_BATCH for arm C,
                                            # args.batch_size for arms A/B), not args.batch_size.
                        ...
                        for _ in range(args.max_episode_length):
                            ...
                            # Gen3v7 U3 — accumulate HardFlow-arm metrics (nlp solves/failures per plan).
                            if is_hardflow:
                        ...
                        for __ in range(0, len(sampled_trajectories_all[i]), plot_samples_every):
                            # Gen3v7 U3: the ARM's fan size. args.batch_size is 4 (arms A/B) but
                            # arm C's fan is HFFM_BATCH, which defaults to 1 — indexing 0..3 into a
                            # length-1 fan is an IndexError. Gen12 had this right
                            # (FM_v3_hardflow_test/eval_FM_v3_hardflow.py:445, `min(batch_size, 4)`);
                            # the Gen3v6 U3 copy regressed it to args.batch_size and only ever ran
                            # HFFM_BATCH=4, so it never fired there. Restored to Gen12's form.
                    ...
                    # Gen3v7 U3 — HardFlow-arm metric summary (nfe accumulated on the sampler).
                    if is_hardflow:
                    ...
                    if config['write_to_file']:
                        ...
                        np.savez(f'{save_path}/{variant_out}.npz',
                                 ...
                                 args=args,
                                 # Gen3v7 U3 — HardFlow-arm metrics (0 / False for arms A/B).
