...
# Core: diffuser_visual_aligning  |  Dataset: ParityAligningDataset (9D)
import argparse
import glob
import json
import os
...
def parse_top_level_args():
    ...
    p.add_argument('--wandb-project', type=str, default='FMPCC-visual-aligning-dpcc')
...
for seed in selected_seeds:
    ...
    if _if_vision:
        from diffuser_visual_aligning.datasets.sequence import ParityAligningDataset
        _DatasetClass = ParityAligningDataset
        print(f'[ train ] dataset=ParityAligningDataset (visual, 9D trajectory)')
    else:
        from diffuser_visual_aligning.datasets.sequence import StateOnlyAligningDataset
    ...
    # ── 4. Trainer — diffuser_visual_aligning Trainer ─────────────────────────
