...
class VisualAgentWrapper:
    ...
    def record_sim_frame(self, env):
        """Fix-18.6: capture a bp/inhand frame DIRECTLY from the env every
        ...
        after env.step(). The frame is pushed into the same
        self.video_frames buffer used by the visual capture path; the
        existing save logic in update_rollout_info dumps it as GIF/MP4.

        Safe defaults: no-ops if record_mode is 'none', if bp_cam is
        missing, or if rendering raises. Never alters policy state."""
