#!/bin/bash
#SBATCH --job-name=visual_avoiding_pipeline_dpcc
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# 1. Training  (train_visual_avoiding_dpcc.sh)
# 2. Evaluation (eval_visual_avoiding_dpcc.sh) — only if training succeeds
# ==============================================================================

SBATCH_DIR="Slurm_Codes/sbatch/diffuser_visual_avoiding"

echo "Launching Visual-DPCC Pipeline..."

# 1. Submit Training Job
TRAIN_ID=$(sbatch --parsable $LOG_OPTS "${SBATCH_DIR}/train_visual_avoiding_dpcc.sh")
echo "Step 1: Training submitted. Job ID: $TRAIN_ID"

# 2. Submit Evaluation Job (success dependency on training)
EVAL_ID=$(sbatch --parsable $LOG_OPTS --dependency=afterok:$TRAIN_ID "${SBATCH_DIR}/eval_visual_avoiding_dpcc.sh")
