#!/bin/bash
#SBATCH --job-name=fm_visual_avoiding_pipeline
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# Visual-FM (Gen7) Pipeline Master Script
# ==============================================================================
# This script chains two Slurm jobs:
# 1. Training  (train_fm_visual_avoiding.sh)
# 2. Evaluation (eval_fm_visual_avoiding.sh) — only if training succeeds
# ==============================================================================

SBATCH_DIR="Slurm_Codes/sbatch/fm_visual_avoiding"

echo "Launching Visual-FM (Gen7) Pipeline..."

# 1. Submit Training Job
TRAIN_ID=$(sbatch --parsable $LOG_OPTS "${SBATCH_DIR}/train_fm_visual_avoiding.sh")
echo "Step 1: Training submitted. Job ID: $TRAIN_ID"

# 2. Submit Evaluation Job (success dependency on training)
EVAL_ID=$(sbatch --parsable $LOG_OPTS --dependency=afterok:$TRAIN_ID "${SBATCH_DIR}/eval_fm_visual_avoiding.sh")
echo "Step 2: Evaluation scheduled (afterok:$TRAIN_ID). Job ID: $EVAL_ID"

echo "--------------------------------------------------------------------------------"
echo "Visual-FM (Gen7) Pipeline submitted successfully."
