...
#
# Example:
#   sbatch pipeline_d3il_baseline.sh ddpm_encdec_vision 42 all
...
echo "D3IL Baseline Pipeline: agent=${AGENT_NAME}  seed=${SEED}  record=${RECORD_MODE}"

# 1. Training
TRAIN_ID=$(sbatch --parsable $LOG_OPTS \
    "${SBATCH_DIR}/train_d3il_baseline.sh" "${AGENT_NAME}" "${SEED}")
...
EVAL_ID=$(sbatch --parsable $LOG_OPTS \
    ...
    "${SBATCH_DIR}/eval_d3il_baseline.sh" "${AGENT_NAME}" "${SEED}" "${RECORD_MODE}")
