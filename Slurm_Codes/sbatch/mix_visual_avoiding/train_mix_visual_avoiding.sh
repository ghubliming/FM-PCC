#!/bin/bash
#SBATCH --job-name=train_mix_visual_avoiding
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ─── Gen16: pick the ML engine arm ──────────────────────────────────────
# $1 = engine  (diffusion | fm | mf | af)   default: fm  (the Gen7 reference arm)
# $2 = seed(s) (optional)              default: $MIX_SEEDS (6 7 8 9 10)
#
#   sbatch train_mix_visual_avoiding.sh mf          # MeanFlow arm, all default seeds
#   sbatch train_mix_visual_avoiding.sh af 7        # alpha-Flow arm, seed 7 only
#   sbatch train_mix_visual_avoiding.sh mf "6 7"    # two seeds, SEQUENTIALLY in this one job
#
# Each arm writes to its OWN checkpoint tree (mix_visual_avoiding_<engine>/...), so the
...
#   against a 24 h wall. Visual avoiding trains a ResNet-18 alongside the U-Net, so one
#   seed at 1e5 steps is already a large fraction of that wall — 5 sequential seeds will not
#   fit. Use mix_visual_avoiding_pipeline.sh instead: it submits ONE JOB PER SEED, so each
...
# ── Gen16 U8 ── ML BONE (generative backbone). Same knob shape as MIX_FILM_MODE:
...
#
#   unet    VisualUNetTwoTime      — the Gen16 baseline (FiLM v1/v2)
#   mf_dit  official MeanFlow DiT  — mf arm only
#   sit     alpha-Flow SiT         — af arm only
#   dit     iMF RoPE DiT           — both arms
#
# On every transformer bone the 64-D visual latent enters as ONE PREPENDED TOKEN.
...
if [ "$(echo $SEEDS | wc -w)" -gt 1 ]; then
    ...
    echo "[ train ]          against the 24 h wall. Prefer mix_visual_avoiding_pipeline.sh,"
...
python mix_visual_avoiding_test/train_mix_visual_avoiding.py \
    --engine "$ENGINE" \
    --seeds $SEEDS \
    ${MIX_AUTO_RESUME:+--auto-resume} \
    ${MIX_SAVE_EVERY:+--save-every $MIX_SAVE_EVERY} \
    --use-wandb \
    --wandb-project FM-PCC-visual-avoiding-gen16
