#!/bin/bash
#SBATCH --job-name=mix_visual_avoiding_pipeline
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# Visual-Mix-ML (Gen16) Pipeline Master Script
# ==============================================================================
# Chains three Slurm jobs for ONE engine arm:
#   1. Gates      (gates_mix_visual_avoiding.sh)   — fidelity + objective sanity, ~minutes
#   2. Training   (train_mix_visual_avoiding.sh)  — only if the gates pass
#   3. Evaluation (eval_mix_visual_avoiding.sh)   — only if training succeeds
#
# Usage:  sbatch mix_visual_avoiding_pipeline.sh <engine> [seeds]
...
#
#   sbatch mix_visual_avoiding_pipeline.sh mf                # all 5 default seeds
#   sbatch mix_visual_avoiding_pipeline.sh mf 6              # seed 6 only (smoke run)
#   sbatch mix_visual_avoiding_pipeline.sh af "6 7 8"        # a subset
#   sbatch mix_visual_avoiding_pipeline.sh mf "6 7" 2        # NFE override (U6)
#   MIX_SEEDS="6 7" sbatch mix_visual_avoiding_pipeline.sh mf   # via env instead
#
...
#
# FAN-OUT (Gen16 multi-seed): this submits ONE train job and ONE eval job PER SEED, all
...
SBATCH_DIR="Slurm_Codes/sbatch/mix_visual_avoiding"
ENGINE="${1:-fm}"
SEEDS="${2:-${MIX_SEEDS:-6 7 8 9 10}}"
# ── Gen16 U6 ── $3 = NFE override, forwarded to every eval in the fan-out. Blank -> config
...
#   sbatch mix_visual_avoiding_pipeline.sh mf "6 7" 2
FLOW_STEPS="${3:-}"
...
# ── Gen16 Fix_9 ── FiLM backbone. Each arm has its OWN knob, MIX_FILM_MODE_<ENGINE>, with a
# bare MIX_FILM_MODE as the all-arms fallback (default v1). Read the block comment above
# base['mix_visual_avoiding_*'] in config/avoiding-d3il-visual-mix.py for what v1/v2 are; the
...
# ── Gen16 U8 ── ML BONE, resolved and narrowed exactly like FILM_MODE above.
...
if [ "$ML_BONE" != "unet" ]; then
    ...
    echo "[ pipeline ]           🔴 A5/A7 cover the bone; run them before trusting a DiT run."
else
    echo "[ pipeline ] ml_bone = unet (default, the Gen16 baseline bone)"
fi
if [ "$FILM_MODE" = "v2" ]; then
    ...
    echo "[ pipeline ]              🔴 v2 has never executed a tensor op on any Gen16 arm — the"
    echo "[ pipeline ]              gates job below (A7) is the first thing that will."
else
    echo "[ pipeline ] film_mode = v1 (default, additive-bias FiLM)"
fi

echo "Launching Visual-Mix-ML (Gen16) Pipeline — engine=$ENGINE seeds='$SEEDS'${FLOW_STEPS:+ K=$FLOW_STEPS} bone=$ML_BONE film=$FILM_MODE ..."

# 1. Gates — ONE job, shared by every seed (seed-independent by construction).
# A0-A9 are all seed-independent, and A7 builds all four arms at whatever film_mode/bone this
# job resolved, so one gates job covers the whole fan-out below.
GATE_ID=$(sbatch --parsable $LOG_OPTS "${SBATCH_DIR}/gates_mix_visual_avoiding.sh")
...
for SEED in $SEEDS; do
    TRAIN_ID=$(sbatch --parsable $LOG_OPTS $EXPORT_OPTS --dependency=afterok:$GATE_ID \
        "${SBATCH_DIR}/train_mix_visual_avoiding.sh" "$ENGINE" "$SEED")
    ...
    EVAL_ID=$(sbatch --parsable $LOG_OPTS $EXPORT_OPTS --dependency=afterok:$TRAIN_ID \
        "${SBATCH_DIR}/eval_mix_visual_avoiding.sh" "$ENGINE" "$SEED" all "$FLOW_STEPS")
    echo "  seed $SEED: eval  scheduled (afterok:$TRAIN_ID). Job ID: $EVAL_ID"
done

echo "--------------------------------------------------------------------------------"
echo "Visual-Mix-ML (Gen16) Pipeline submitted — engine=$ENGINE seeds='$SEEDS'."
