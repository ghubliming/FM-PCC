#!/bin/bash
#SBATCH --job-name=hffm_gates
#SBATCH --nodes=1                   # Run on a single node
#SBATCH --ntasks=1                  # Run a single task
#SBATCH --cpus-per-task=8           # Number of CPU cores per task
#SBATCH --mem=16G                   # Total memory
#SBATCH --gres=gpu:1                # Request 1 GPU
#SBATCH --time=01:00:00             # Time limit (~2x expected; gates take minutes)
...
# 4) Gen12 pre-flight gates (PLAN §4 steps 2). No checkpoint, no dataset needed.
#    G0 dof layout vs HardFlow, G1 tau direction, G2 prox-NLP feasibility,
#    G3 end-to-end constrained sampler on a stub field.
#    A non-zero exit here means DO NOT PROCEED (PLAN §4).
python FM_v3_hardflow_test/gates_hardflow.py --flow-steps "${HFFM_K:-5}"

echo "Gates completed successfully."
