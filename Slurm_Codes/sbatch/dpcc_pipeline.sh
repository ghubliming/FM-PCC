...
# ==============================================================================
# DPCC (Diffuser) Pipeline Master Script
# ==============================================================================
# This script chains three Slurm jobs:
# 1. Training (train_dpcc_job.sh)
# 2. Evaluation (eval_dpcc_job.sh) - Only if Training succeeds
# 3. Load Results (load_results_dpcc_job.sh) - Only if Evaluation succeeds
# ==============================================================================

# Ensure we are in the right directory
SBATCH_DIR="Slurm_Codes/sbatch"

echo "Launching DPCC Pipeline..."

# 1. Submit Training Job
# --parsable makes sbatch only return the Job ID
TRAIN_ID=$(sbatch --parsable $LOG_OPTS "${SBATCH_DIR}/train_dpcc_job.sh")
...
EVAL_ID=$(sbatch --parsable $LOG_OPTS --dependency=afterok:$TRAIN_ID "${SBATCH_DIR}/eval_dpcc_job.sh")
...
LOAD_ID=$(sbatch --parsable $LOG_OPTS --dependency=afterok:$EVAL_ID "${SBATCH_DIR}/load_results_dpcc_job.sh")
