...
# 4) Run AlphaFlow (Gen3v7) Results Analysis
cd "$REPO"

# Load results automatically detects the plan path using the experiment parser.
# Same K grid as eval_alphaflow.sh — results live in a per-K directory, so aggregating
# without the matching --flow-steps would silently report only the K=2 default.
FLOW_STEPS_GRID="${AF_FLOW_STEPS:-1 2 5 10}"
for K in $FLOW_STEPS_GRID; do
    echo "[ load_results ] K = $K"
    python FM_v3_alphaflow_test/load_results_flow_matching_v3_alphaflow.py --flow-steps "$K"
done
