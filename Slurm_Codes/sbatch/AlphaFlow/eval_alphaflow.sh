...
# 4) Run AlphaFlow (Gen3v7) Evaluation
cd "$REPO"

# Evaluation reads seeds and variants from config/projection_eval.yaml.
#
# ⚠️ MATCHED BUDGET OR NOTHING (PLAN §8 / fix_7.3 §9). The entire Gen13 claim died because
# one hard-coded k_steps=10 made the decisive control unrunnable, and the confound survived
# four rounds of analysis. The K grid is therefore built in HERE, from day one — not left
# as a config edit somebody has to remember. Each K writes its own results directory
# (flow_steps_v3 is watched as 'K' in the plan exp_name), so nothing overwrites anything.
#
# Override the grid without editing this file:  AF_FLOW_STEPS="2" sbatch ...
FLOW_STEPS_GRID="${AF_FLOW_STEPS:-1 2 5 10 20}"
...
for K in $FLOW_STEPS_GRID; do
    ...
    echo "[ eval ] K = $K   ($(date))"
    echo "================================================================================"
    python FM_v3_alphaflow_test/eval_flow_matching_v3_alphaflow.py --flow-steps "$K"
done
