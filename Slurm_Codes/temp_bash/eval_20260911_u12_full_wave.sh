#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Gen15 U12 — full wave on `corridor_ball_v2` (the small on-path ball), 3 engines, TWO TIERS
#
...
#
# BASELINE: the SAME six arms already exist on U11's `corridor_ball` (geo tag `_hgb`) from
# jobs 25605-25610, and on `corridor_hg` from batch_uav_20260910_092309. `_hgb2` is a third
# sibling geo folder under the same candidates, so a DA pairs all three on the `geo` axis.
#
# ⚠️ U11's wave scored S&C 0/34 and collision_free 0.000 on 32 of 34 cells. The 12-minute
# injection test on THIS geometry (job 25657) reproduced that on mf K=2: `diffuser` 27.1
# violations (the ball binds) but `dpcc-t` collision_free 0.000, with `min_z == final_z` on
# all 10 rollouts -- the projector never moved the plan vertically. Suspected cause: the
# corridor action bound (`action_bounds='auto'` -> dy,dz capped at ~2.2e-05 m/step, because
# the corridor expert flies at constant y and z). Adding `dpcc-t-bounds_free` to either tier
# tests that in the same wave for ~2 extra minutes per job.
#
