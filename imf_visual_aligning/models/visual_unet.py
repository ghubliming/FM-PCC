...
class VisualUNet(nn.Module):
    ...
    Backbone: imf_visual_aligning.models.unet1d_temporal_cond.UNet1DTemporalCondModel
    ...
    def __init__(self, config):
        ...
        from imf_visual_aligning.models.unet1d_temporal_cond import UNet1DTemporalCondModel
    ...
    def forward(self, x, cond, t, returns=None, use_dropout=True, force_dropout=False, h=None):
        """
        x:    (B, T, 9)  — noisy trajectory
        cond: dict with 'visual': (bp_imgs, inhand_imgs, obs_seq)
        t:    (B,) diffusion timestep indices
        h:    (B,) iMF mean-flow step size (threaded to backbone h_mlp)
        """
