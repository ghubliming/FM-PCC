...
def parse_top_level_args():
    ...
    p.add_argument('--wandb-project', type=str, default='FM-PCC-visual-avoiding-gen9')
...
for seed in selected_seeds:
    args = Parser().parse_args(experiment='fm_visual_avoiding', seed=seed)
    ...
    # ── 1. Dataset ────────────────────────────────────────────────────────────
    # Visual (if_vision=True): ParityAvoidingDataset — 6D [act(2)|des_xy(2)|c_xy(2)]
    # Gen9 Epoch 2: visual-avoiding only. Non-visual is out of scope.
    _if_vision = getattr(args, 'if_vision', True)
    if not _if_vision:
        raise NotImplementedError(
            "fm_visual_avoiding is visual-only (Gen9 Ep 2). "
            "Set if_vision=True in the fm_visual_avoiding config, or implement "
    ...
    from fm_visual_avoiding.models.visual_unet import VisualUNet
    ...
    # ── 4. Trainer — fm_visual_avoiding Trainer ───────────────────────────────
