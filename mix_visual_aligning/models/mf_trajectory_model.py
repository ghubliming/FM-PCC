...
class MFTrajectoryModel(nn.Module):
    def __init__(
        ...
        # ── Gen14 GRAFT — visual arm (shape borrowed from Gen8 "Option A") ──
        ...
        # objective, JVP and sampler above/below are UNCHANGED — this is a
        # backbone swap, exactly like 'dit'/'mf_dit'.
        ...
        if if_vision:
            ...
            self.velocity_net = MFDiTTrajectory(
                horizon=seq_len,
        ...
        print(f'[ MFTrajectoryModel ] backbone={imf_backbone}  vision={self.if_vision}  unet_width(freq_dim)={freq_dim}  '
    ...
    def forward(
        ...
        # Gen14: the visual backbone honours the same (u, v) contract — it returns a
        # tuple only when dual_head is on, so it falls through the two branches below
        # exactly like Flow_matcher_U_Net_v2 does. No extra case needed.
