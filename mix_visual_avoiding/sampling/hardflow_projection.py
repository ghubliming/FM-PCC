"""Gen16 — HardFlow's `hardflow_new` constrained sampler, on the Gen16 visual arms.

🔴 THIS FILE IS A COPY of `mix_visual_aligning/sampling/hardflow_projection.py` (Gen14 U7)
with the package prefix repointed and ONE functional edit: `encode_visual_cond()` now
unpacks the camera payload through `models/visual_spec.py` instead of destructuring a
hardcoded two-camera tuple, and routes the `fm` arm to the raw-image cond key it actually
consumes. Everything else — the NLP, `TrajectoryLayout`, the prox schedule, the activation
gate, the pull-back, `ENGINE_INIT_NOISE` — is byte-identical to Gen14's. Fix upstream and
re-copy; do not diverge here. `gates_hardflow_mix_visual_avoiding.py::gate_h0` pins the
byte identity of the unchanged regions.

Gen14's own header follows verbatim.
────────────────────────────────────────────────────────────────────────────────

Gen14 U7 — HardFlow's `hardflow_new` constrained sampler, on the Gen14 visual arms.

Port of `HardFlow/hardflow/models_flow/flow_policy.py::hardflow_new_forward`
(+ `hardflow_formulate`) into the FMPCC codebase.

LINEAGE: Gen12 (`flow_matcher_v3_hardflow/`) -> Gen3v6 U3+fix_4
(`flow_matcher_v3_meanflow/sampling/hardflow_projection.py`) -> Gen3v7 U3
(`flow_matcher_v3_alphaflow/sampling/hardflow_projection.py`) -> here.

🔴 THIS FILE IS A COPY. It was produced by `cp` from the Gen3v7 file and then edited in
exactly the four places listed below. The NLP (`HardFlowNLP`), the dof layout
(`TrajectoryLayout`), the prox schedule, the activation gate and the pull-back are
**byte-identical** to Gen3v7's. Do not "improve" them here — fix them upstream and re-copy,
the way Gen3v6 -> Gen3v7 did. `gates_hardflow_mix_visual.py::gate_h0` pins that byte
identity for the unchanged regions.

WHAT IS GEN14-SPECIFIC (the whole diff against Gen3v7):

  1. **`HardFlowPolicy` is dropped.** Gen3v6/v7 drive the sampler through diffuser's
     `Policy`/`Trajectories`; Gen14's closed loop is `VisualAgentWrapper`
     (`eval_mix_visual_avoiding.py`), which calls the sampler directly. The 126-line
     policy class and its `from .policies import Trajectories` import are therefore not
     copied. Nothing else referenced them.

  2. **`two_time` flag on `HardFlowSampler`** — Gen14 hosts THREE engines with two
     different velocity signatures:
         fm  (Gen7)    `_predict_velocity(x, cond, t, returns=None)`         <- no h
         mf  (Gen3v6)  `_predict_velocity(x, cond, t, h=None, returns=None)`
         af  (Gen3v7)  `_predict_velocity(x, cond, t, h=None, returns=None)`
     Gen12's copy calls the first form, Gen3v6/v7's the second. Rather than keep two
     files, `two_time` selects the call. `two_time=True` passes `h=0` EXPLICITLY (the
     mean-flow identity `u(x,t,0) = v(x,t)`); `two_time=False` omits `h` entirely, which
     is Gen12's call verbatim. **`diffusion` is not a host** — it has no velocity field at
     all; the eval refuses it before this class is constructed.

  3. **The visual latent must survive into the network call.** This is the one place a
     verbatim copy would have been silently WRONG. Gen3v6/v7 do
     `cond_net = {k: v for k, v in cond.items() if not isinstance(k, str)}` to strip
     diffuser's string bookkeeping keys. In Gen14 the visual conditioning IS a string key
     (`cond['visual_latent']`, `visual_unet_twotime.py:185`), and `_project_cond` returns
     `None` when it is absent — no exception, no warning. A verbatim port would have run
     every HardFlow rollout **blind**, producing plausible-looking numbers from an
     unconditioned field. `_VISUAL_COND_KEYS` is now an explicit allow-list.

  4. **`init_noise_scale` re-derived from Gen14's own samplers** (fix_4's standing rule —
     read it from the HOST generation, never inherit it):
         fm  -> 0.5   (`fm_diffusion.py:164`,  `0.5 * torch.randn`)
         mf  -> 1.0   (`mf_diffusion.py:204`,  `torch.randn`)
         af  -> 1.0   (`af_diffusion.py:260`,  `torch.randn`)
     The argument stays REQUIRED (no default): a wrong scale is silent. `ENGINE_INIT_NOISE`
     below is the single source of truth and `gate_h3` pins it numerically.

See `logs_in_develop/Gen3v7_AlphaFlow/init/PORT_GUIDE_hardflow_into_Gen3v7.md` for the
port rules this file follows, and `logs_in_develop/Gen14/U7/` for this port's changelog.
...
-----------------------------------------------------------------------------
The NLP needs an *instantaneous* velocity `v(x, tau)`.  α-Flow's net emits the
*interval average* `u(x, r, t)`, so the port queries it at h = 0, where the
mean-flow identity grounds the field exactly:  `u(x, t, 0) = v(x, t)`.

Gen3v6 relied on that identity holding *structurally*.  α-Flow additionally
TRAINS it: `af_diffusion.py:694` draws `fm_mask = rand(B) < self.af_ratio_fm`
and forces that fraction of every batch to `r == t` (h = 0) with
`u_tgt = v_inst` — the pure flow-matching anchor.  The default is
`af_ratio_fm = 0.5` (`af_diffusion.py:94`), i.e. HALF of every batch is direct
supervision of exactly the corner this sampler queries.  So the h=0 field here
is better supported than in Gen3v6, not merely assumed.

⚠️ If a checkpoint was trained with `af_ratio_fm = 0`, the anchor was never
trained and this whole arm rests on an untrained corner of the field.  Check the
loadpath token `_rf{af_ratio_fm}` before believing any number.

`h = 0` is passed EXPLICITLY (never relying on an `h=None` backbone default) so a
future default change cannot silently swap the grounded field for the interval
average.  `gates_hardflow_alphaflow.py::gate_h1` pins this.
...
`0.5 * randn`.  Gen3v6's MeanFlow (`mf_diffusion.py:204`) and Gen3v7's α-Flow
(`af_diffusion.py:260`, *"sigma=1.0 to match q_sample training noise"*) both
start from plain `randn` — sigma=1.0.  Gen3v6's original U3 port copied Gen12's
0.5 verbatim, so arm C ran at HALF the trained noise scale for an entire
K-sweep: an out-of-distribution tau=0 state, and arms B/C not sharing a start
distribution — the exact thing this sampler is supposed to guarantee. That cost
a full re-run (Gen3v6 fix_4) and is the single most expensive mistake in this
port's history.

🔴 DO NOT read the scale from `flow_matcher_v3_alphaflow/models/diffusion.py:183,302`.
That is the LEGACY FMv3ODE class living in the same folder and it is still on
`0.5 * randn`. It is not the engine this arm runs against. Read `af_diffusion.py`
— the α-Flow engine — and nothing else.
...
`HardFlowPolicy` defaults it to Gen3v7's 1.0 and the eval driver passes it
...
# Gen16 — the camera payload's shape is a task property; see models/visual_spec.py.
...
#
# The string keys that carry image conditioning and must SURVIVE the cond filter in
# `HardFlowSampler.sample`. Mirrors `VisualUNetTwoTime._project_cond`
# (`visual_unet_twotime.py:182-189`), which prefers 'visual_latent' and falls back to the
# raw 'visual' tuple.
_VISUAL_COND_KEYS = frozenset({'visual_latent', 'visual'})

# U7 delta 4 (fix_4's standing rule: read the scale off the HOST generation's own sampler,
# never inherit it). Single source of truth; `gate_h3` pins these numerically.
#   fm -> fm_diffusion.py:164  `x = 0.5 * torch.randn(shape, ...)`
#   mf -> mf_diffusion.py:204  `x = torch.randn(shape, ...)  # sigma=1.0`
#   af -> af_diffusion.py:260  `x = torch.randn(shape, ...)  # sigma=1.0`
# 'diffusion' is deliberately ABSENT — it is a DDPM reverse chain with no velocity field,
# so it is not a HardFlow host and `resolve_engine_hf` refuses it.
ENGINE_INIT_NOISE = {'fm': 0.5, 'mf': 1.0, 'af': 1.0}

# Which arms expose the two-time `_predict_velocity(..., h=...)` signature.
ENGINE_TWO_TIME = {'fm': False, 'mf': True, 'af': True}


def resolve_engine_hf(engine):
    """(init_noise_scale, two_time) for a Gen14 arm, or a loud refusal.

    The refusal is the point: `diffusion` (Gen6V4 DDPM) has no velocity field, so
    `hardflow_new` — which integrates `v = f(x, t)` outside the solver — is not merely
    unsupported on it, it is undefined. Failing here keeps that fact one line from the
    config instead of surfacing as an AttributeError deep inside a GPU allocation.
    """
    key = str(engine).lower().strip()
    if key not in ENGINE_INIT_NOISE:
        raise ValueError(
            f"HardFlow is not available for engine '{key}'. Hosts are "
            f"{sorted(ENGINE_INIT_NOISE)} — the flow-matching arms. The 'diffusion' arm is a "
            f"DDPM reverse chain with no velocity field, so `hardflow_new` is undefined on it "
            f"(see Gen14 U7 changelog §2).")
    return ENGINE_INIT_NOISE[key], ENGINE_TWO_TIME[key]


def encode_visual_cond(model, cond):
    """Turn the eval policy's raw cond into the cond the sampler needs.

    Mirrors `VisualMeanFlow.forward` / `VisualAlphaFlow.forward` / `VisualFlowMatching.forward`
    — same unpack, same `[:, -1]` snapshot — because the HardFlow sampler bypasses
    `model.forward()` (it replaces `p_sample_loop` wholesale) and would otherwise never
    reach the repacking those wrappers do.

    Two-time arms (mf/af) get a PRE-ENCODED `visual_latent`: encoding once here rather than
    per ODE step is what the wrappers do and is numerically identical (the encoder is
    deterministic in eval mode — GroupNorm, no dropout — and the images are constant across
    the ODE loop).

    🔴 Gen16 — the `fm` arm gets `visual` (RAW IMAGES) instead. `VisualFlowMatching` has no
    `_encode_once`; only the two-time wrappers define it, because only they need the latent
    to be a JVP constant. Gen14's version called `model._encode_once(...)` unconditionally,
    which would raise AttributeError the first time arm C ran on the fm host. The fm path
    reaches the encoder through `VisualUNet.forward`'s own `cond['visual']` branch, so
    handing it images is both correct and what the non-HardFlow fm path already does.
    `_VISUAL_COND_KEYS` already allow-lists BOTH keys, so the blind-field guard below
    accepts either.
    ...
    if not isinstance(cond, dict):
        return cond
    if any(k in cond for k in _VISUAL_COND_KEYS):
        return cond                                  # already encoded/packed upstream
    if 0 in cond and isinstance(cond[0], tuple):
        cam_imgs, obs_seq = visual_spec.split_visual(cond[0])
        snap_obs = obs_seq[:, -1]
        if hasattr(model, '_encode_once'):           # mf / af — latent, JVP-safe
            return {0: snap_obs, 'visual_latent': model._encode_once(*cam_imgs)}
        # fm — the backbone encodes from raw images itself (VisualUNet.forward).
        return {0: snap_obs, 'visual': visual_spec.pack_visual(cam_imgs, obs_seq)}
    return cond
...
class HardFlowSampler:
    ...
    def __init__(self, model, layout, nlp, init_noise_scale, device='cuda',
                 activation_threshold=0.0, verbose=False, two_time=True):
        ...
        # U7 delta 2: which `_predict_velocity` signature this host exposes.
        #   True  (mf/af) -> pass h=0, the mean-flow grounding u(x,t,0) = v(x,t)
        #   False (fm)    -> omit h; Gen7's FM net already emits the instantaneous v
        # Getting this wrong is a TypeError on the first call, not a silent degradation,
        # which is why it is allowed to have a default at all.
        self.two_time = bool(two_time)
        # U7 delta 3: read once so `sample()` can hard-fail on a blind visual rollout.
        # 🔴 `if_vision` lives on the BACKBONE (`visual_unet_twotime.py:72`), not on the
        # diffusion wrapper — `visual_fm_diffusion.py:47` reads it as `self.model.if_vision`.
        # Probing the wrapper alone silently yields False and disarms the guard.
        self.model_is_visual = bool(
            getattr(getattr(model, 'model', None), 'if_vision', False)
            or getattr(model, 'if_vision', False))
        self.device = device
    ...
    def _velocity_batch(self, X, tau, s0_all, cond, returns):
        ...
        with torch.no_grad():
            # Gen3v7 U3: this is a VERBATIM Gen12/Gen3v6 port EXCEPT this one line. Gen12 feeds an
            # instantaneous-velocity FM model, which returns v(x,t) directly. α-Flow (AlphaFlowODE)
            # returns the interval average u(x,r,t); we query it at h=0 (r=t), where the mean-flow
            # identity gives u(x,t,0) = v(x,t) EXACTLY. α-Flow does not merely inherit that identity
            # — it TRAINS it: af_diffusion.py:694 forces `af_ratio_fm` (default 0.5) of every batch
            # to r==t with u_tgt = v_inst, so half of training is direct supervision of this exact
            # query. The field handed to the NLP/endpoint is therefore identical to Gen12's and the
            # projection math is unchanged. h=0 is passed EXPLICITLY (not relying on the h=None→0
            # backbone default) so a future default change can't silently reintroduce the
            # interval-average field. `_predict_velocity(x, cond, t, h=None, returns=None)` at
            # af_diffusion.py:221 — same signature as Gen3v6's, so this call is unmodified.
            #
            # U7 delta 2: the `fm` arm (Gen7 flow matching) has no `h` in its signature
            # (`fm_diffusion.py:71`) and its net already emits the instantaneous velocity, so
            # that branch is Gen12's original call verbatim. mf/af keep the Gen3v6/v7 call.
            if self.two_time:
    ...
    def sample(self, cond, flow_steps, batch_size=1, returns=None):
        ...
        # that shares the folder. This is what actually makes arms A, B and C start
        # from the same distribution. PLAN §3.3 direction: FMv3 integrates
        ...
        #
        # 🔴 U7 delta 3 — THE ONE PLACE A VERBATIM COPY WOULD HAVE BEEN SILENTLY WRONG.
        # Gen3v6/v7 are state-only, so `not isinstance(k, str)` correctly strips diffuser's
        # bookkeeping keys and loses nothing. Gen14 is VISUAL and its conditioning travels
        # under a string key: `cond['visual_latent']` (B, 128), consumed by
        # `visual_unet_twotime.py:185`. That method returns **None** when the key is absent —
        # no exception, no warning — so the unmodified line would have run every HardFlow
        # rollout with no image conditioning at all and produced plausible numbers from a
        # blind field. The allow-list below is explicit for that reason; extend it if the
        # visual cond contract ever grows another key.
        cond_net = {k: v for k, v in cond.items()
                    if not isinstance(k, str) or k in _VISUAL_COND_KEYS}
        if self.model_is_visual and not any(k in cond_net for k in _VISUAL_COND_KEYS):
            raise ValueError(
                'HardFlowSampler: host model has if_vision=True but `cond` carries none of '
                f'{sorted(_VISUAL_COND_KEYS)}. Pass the ENCODED conditioning (see '
                '`encode_visual_cond`) — running blind is not an allowed silent fallback.')
        returns_net = returns
...
# ── Gen14 U7 — construction entry point ───────────────────────────────────────────────
#
# Gen3v6/v7 build the layout, the NLP and the sampler inside `HardFlowPolicy.__init__`
# (`flow_matcher_v3_alphaflow/sampling/hardflow_projection.py:626-668`). Gen14 has no
# `Policy` — its closed loop is `VisualAgentWrapper` — so that constructor body is lifted
# here verbatim, minus the diffuser-Policy bookkeeping (`preprocess_fn`, `test_ret`,
# `prev_observations`) that only the `Policy.__call__` path used. The three objects and the
# order they are built in are unchanged.

def build_hardflow_sampler(model, normalizer, horizon, transition_dim, action_dim,
    ...
    `.normalizers['observations'|'actions'].mins/.maxs` interface Gen3v7's constructor
    reads, so the mins/maxs block below is unmodified.

    `engine` selects `init_noise_scale` and `two_time` through `resolve_engine_hf`, which
    refuses 'diffusion' (U7 delta 2/4). Nothing else about the arm is consulted.
    """
    assert goal_dim == 0, (
        'Gen12 targets goal_dim=0. A goal-conditioned env would need the goal columns '
        'carried through the dof vector.')
    init_noise_scale, two_time = resolve_engine_hf(engine)

    state_dim = transition_dim - action_dim
    layout = TrajectoryLayout(horizon, action_dim, state_dim)

    # Limits over the FULL transition vector [actions | observations], in the same order
    # the trajectory uses. Mirrors ProjectionNormalizer.
    ...
    print(f'[ hardflow ] engine={engine}  init_noise_scale={init_noise_scale}  '
          f'two_time={two_time}  act_thr={sampler.activation_threshold}  '
          f'visual={sampler.model_is_visual}')
    return layout, nlp, sampler
