...
class AFTrajectoryModel(nn.Module):
    def __init__(
        ...
        if if_vision:
            ...
            if imf_backbone == 'unet':
            ...
            elif imf_backbone in ('sit', 'dit'):
                from .visual_dit_twotime import VisualDiTTwoTime
                # 'sit' is alpha-Flow's own bone; 'dit' is the RoPE iMF bone, disambiguated
                # per arm because the mf and af copies are separate classes with separate
                # provenance.
                variant = 'sit' if imf_backbone == 'sit' else 'dit_af'
                ...
                raise ValueError(
                    f"[ AFTrajectoryModel ] if_vision=True with imf_backbone='{imf_backbone}' "
                    f"is not supported (want 'unet', 'sit' or 'dit'). The mf-only bone "
                    f"'mf_dit' belongs to MFTrajectoryModel."
                )
        ...
        print(f'[ AFTrajectoryModel ] backbone={imf_backbone}  vision={self.if_vision}  unet_width(freq_dim)={freq_dim}  '
