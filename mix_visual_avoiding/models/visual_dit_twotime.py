...
_BONES = {
    'mf_dit': ('mix_visual_avoiding.models.mf_dit_official_trajectory', 'MFDiTOfficialTrajectory'),
    'sit':    ('mix_visual_avoiding.models.af_sit_trajectory',          'AFSiTTrajectory'),
    'dit_mf': ('mix_visual_avoiding.models.mf_dit_trajectory',          'MFDiTTrajectory'),
    'dit_af': ('mix_visual_avoiding.models.af_dit_trajectory',          'AFDiTTrajectory'),
}
...
class VisualDiTTwoTime(nn.Module):
    ...
    Trajectory dimension comes from `visual_spec`, exactly as `VisualUNetTwoTime` — the
    same module, so the two bones cannot disagree. Never reads config.obs_dim; that field
    can be a stale placeholder (fix_5 lesson).

    Vision: MultiImageObsEncoder (single ResNet, bp-cam) -> 64D latent -> ONE token.
    """

    # Gen16 — from visual_spec, the single source of truth. Mirrored onto the class so
    # `VisualDiTTwoTime.TRANSITION_DIM` keeps working for callers that read it off the class.
    # Reading the SAME module as VisualUNetTwoTime is what makes the U-Net-vs-DiT
    # comparison architecture-controlled by construction rather than by inspection.
    ...
    def __init__(self, config, variant, dual_head=False, interval_cfg=False, **dit_kwargs):
        ...
        # ── 1. Vision encoder ─────────────────────────────────────────────────
        # 🔴 Gen16: the encoder is built by `visual_spec.build_obs_encoder`, the SAME call
        # `VisualUNetTwoTime` makes. Gen14 kept these two blocks byte-identical by hand and
        # warned that any drift silently breaks the U-Net-vs-DiT comparison this unit exists
        # to make; sharing one constructor makes that drift unrepresentable instead.
        if self.if_vision:
