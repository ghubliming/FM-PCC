"""Gen16 — the engine dispatch table. THE only place an arm branches.

Copied from Gen14 (`mix_visual_aligning/models/engine_registry.py`) with the package
prefix repointed and NOTHING else changed: the four arms, their classes, their trainers,
their nfe keys and the alias map are the aligning ones verbatim. The TASK differs
(6-D single-camera avoiding vs 9-D dual-camera aligning) but the task lives entirely in
`visual_spec.py` and the dataset, which is exactly why this table did not have to change.

`engine` is a config key: one of 'diffusion' | 'fm' | 'mf' | 'af'. Everything that differs
between arms is declared here, so `train_mix_visual_avoiding.py` and
`eval_mix_visual_avoiding.py` contain no `if engine == ...` chains.
...
ENGINES = {
    ...
    'diffusion': dict(
        ...
        source      = 'diffuser_visual_aligning/ (Gen6V4) via mix_visual_aligning/ (Gen14)',
    ),
    ...
    'fm': dict(
        ...
        source      = 'fm_visual_aligning/ (Gen7) via Gen14',
    ),
    ...
    'mf': dict(
        ...
        nfe_key     = 'flow_steps_v3',
        source      = 'flow_matcher_v3_meanflow/ (Gen3v6) via Gen14',
    ),
    ...
    'af': dict(
        ...
        trainer     = _U + 'training_twotime.Trainer',
        nfe_key     = 'flow_steps_v3',
        source      = 'flow_matcher_v3_alphaflow/ (Gen3v7) via Gen14',
    ),
}
