...
# Gen16 — the eval yaml this package reads when the eval script has not published
# FMPCC_PROJ_CFG (i.e. a training run, which snapshots the config for provenance only).
# See snapshot_configs() below for why this is not hardcoded at the call site.
_DEFAULT_EVAL_YAML = 'config/visual_avoiding_mix_eval.yaml'
...
class Parser(argparse.ArgumentParser):
    ...
    def __init__(self, *args, savepath='', exe_name=None, **kwargs):
    ...
    def snapshot_configs(self, args):
        ...
        # 2. Copy the eval yaml THIS package actually reads.
        #
        # 🔴 Gen16 — the path is taken from FMPCC_PROJ_CFG when the eval published it, and
        # falls back to Gen16's default. Hardcoding one filename here is a bug this repo has
        # now shipped twice: Gen9 inherited `config/visual_aligning_eval.yaml` from the
        # aligning package and snapshotted a DIFFERENT TASK's config into every avoiding run
        # (Gen9 U-fix), and Gen3v6's FIX_9_CFG_PROVENANCE found the same class of error where
        # the snapshot named a yaml the eval never opened. Reading the env var means the
        # snapshot follows `--config` overrides and K/threshold sweeps for free.
        yaml_path = os.environ.get('FMPCC_PROJ_CFG') or _DEFAULT_EVAL_YAML
        ...
        # Cleanup: remove a stale WRONG-TASK snapshot left by any earlier run in this
        # directory. `visual_aligning_eval.yaml` is never legitimate content for this
        # package, so its presence can only be the old hardcoded-path bug.
