...
#
# ── Gen16 ── 'ema' re-added (mirrors Gen3v6 Fix2/U6). Gen14 inherited the DPCC-legacy
# 6-field tuple, which exposes ONLY the raw weights, so an eval had no way to deploy the
# EMA weights without reaching into `trainer`. Gen16's trainers select `state_best` on the
# EMA loss (see utils/training.py::test), so the EMA weights are the ones that must be
# reachable — and `eval_use_ema` needs both loaded to be a real switch. Both are loaded
# regardless; the config key only picks which the rollout uses.
...
def load_diffusion(*loadpath, epoch='best', device='cuda:0', seed=None):
    ...
    return DiffusionExperiment(dataset, trainer.model.model, trainer.model,
