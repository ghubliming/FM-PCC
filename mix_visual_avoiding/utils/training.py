...
class Trainer(object):
    def __init__(
        ...
        if train_test_split == 1:
            ...
            # ── Gen16 ── EPISODE-LEVEL SPLIT when the dataset offers one.
            # `ParityAvoidingDataset` yields overlapping sliding windows, so a random
            # WINDOW split puts near-duplicate windows from the same episode on both
            # sides of the train/test line. The test loss then reads far too low and
            # `state_best` is selected against a leaked set. Gen9 U4 diagnosed this;
            # `episode_split()` cuts on episode boundaries instead.
            #
            # 🔴 It is also what makes the four Gen16 arms comparable: the split is
            # DETERMINISTIC (episode index, no RNG), so diffusion/fm (this trainer) and
            # mf/af (training_twotime.py) train and validate on exactly the same
            # windows. Gen14 could not say that — its two trainers used a seeded and an
            # unseeded random split respectively, which PLAN §4 had to document as a
            # standing cross-arm confound on test_loss. Here the confound is gone.
            #
            # The random-split branch is kept verbatim for any dataset without the method.
            if hasattr(self.dataset, 'episode_split'):
                ...
                print(f'[ Trainer ] episode-level split: {len(train_idx)} train / '
                      f'{len(test_idx)} test windows (deterministic)')
            else:
    ...
    def train(self):
        ...
        # ── Gen16 (Gen9 B8) ── persist the FINAL weights.
        # `self.step` is incremented AFTER the periodic-save check, so the loop only ever
        # saves at steps 0, save_freq, ..., n_train_steps - save_freq. The last
        # `save_freq` steps of every run were being thrown away. One extra save fixes it.
        self.save(self.step)

    def test(self, n_test=100):
        # ── Gen16 (Gen9 U4 Fix1) ── score the EMA weights, because the EMA weights are
        # what eval DEPLOYS (`eval_use_ema` defaults True on every arm). Upstream DPCC
        # scored `self.model` AND deployed `self.model`, which was self-consistent;
        # scoring raw while deploying EMA is not — `state_best` would then be the step
        # that was best for a network nobody runs. Raw-optimal != EMA-optimal.
        #
        # 🔴 Both Gen16 trainers do this, so the four arms' `state_best` are selected
        # under one criterion. Do not change it in only one of them.
        # Revert path (DPCC purity): use `self.model` here AND set eval_use_ema=False.
        # Never mix the two — that re-creates the mismatch.
