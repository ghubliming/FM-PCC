...
#
# Output:  logs/avoiding-d3il/visual_aligning_dpcc/<exp>/results/<seed>/
...
from diffuser_visual_avoiding.sampling.projection import Projector
...
def setup_dpcc_projector(args, config, obs_normalizer, act_normalizer, variant,
    ...
    Gen9 Ep 2 Fix-3: avoiding trajectory is 6D, not 9D.
    ...
    if hasattr(obs_normalizer, 'mins') and len(obs_normalizer.mins) > _target_obs_dim:
        from diffuser_visual_avoiding.datasets.normalization import LimitsNormalizer as _LN
...
def _hs_xy_draw(ax, hs, enlarge, xlim, ylim, dashed=False):
    ...
    Infeasight half-plane fill uses a polygon built from the two clipped boundary
...
def plot_geo_constraints(geo_name, geo_config, out_dir, is_tightened=False):
    ...
    # Halfspace XZ projection — boundary plane clips to x-extent of the boundary
    # line inside the display viewport.  Shown as a semi-transparent vertical band
    # spanning full z, identical style to the 2D obstacle band.
    for _hs_xz in halfspace_list:
        ...
        _hxx1 += enlarge * _hxnx;  _hxy1 += enlarge * _hxny   # tighten boundary
        ...
        # y values at clipped endpoints — annotate so reader knows which y the band maps to
...
def check_trajectory_constraints(c_pos_traj, act_traj, geo_config, enlarge=0.0):
    ...
    # Per-step violation magnitude arrays (zero = satisfied)
    b_mag  = np.zeros(T)   # max distance outside bounds at each step
    hs_mag = np.zeros(T)   # max distance past halfspace boundary
    ob_pen = np.zeros(T)   # max penetration depth into obstacle sphere

    # Per-step margin to nearest constraint boundary (inf until overwritten)
    ...
    if 'bounds' in ct and 'workspace_bounds' in geo_config:
        ...
        viol_lo  = np.maximum(0.0, lb - pos)           # (T, 3) below lb
        viol_hi  = np.maximum(0.0, pos - ub)           # (T, 3) above ub
        b_mag    = np.maximum(viol_lo, viol_hi).max(axis=1)     # (T,)
        ok       = b_mag <= 1e-6
        margins  = np.minimum(pos - lb, ub - pos).min(axis=1)   # (T,) min dist to any wall
    ...
    if 'halfspace' in ct:
        for hs in geo_config.get('halfspace_constraints', []):
            ...
            if nl < 1e-9:
                continue
            nx, ny = nx / nl, ny / nl
            x1 += enlarge * nx;  y1 += enlarge * ny   # tighten: shift boundary into feasible region
            sd = nx * (pos[:, 0] - x1) + ny * (pos[:, 1] - y1)   # (T,) +ve = feasible
            ok = sd >= -1e-6
    ...
    # ── Innovative metrics ─────────────────────────────────────────────────────
    # Longest consecutive safe streak
    max_streak = cur = 0
    for v in any_viol:
    ...
    first_viol = int(viol_idx[0]) if len(viol_idx) else -1   # -1 = no violation

    # Dynamics consistency: ||c_pos[t+1] - c_pos[t] - act[t]|| per step
...
class VisualAgentWrapper:
    ...
    def update_rollout_info(self, info):
        ...
        if self.geo_config and self.curr_rollout_c_pos:
            _cmetrics = check_trajectory_constraints(
                ...
                self.geo_config,
                0.0)
        # Add planning violation rate (mean over replan steps this rollout)
    ...
    def _export_rollout_realtime(self, rollout_idx):
        ...
        try:
            ...
            if all_cands_list:
                ...
                # For nominal/relaxed variants draw one layer at _enlarge (which is 0).
                _c_layers = [(0.0, False)]
                if self.is_tightened and _enlarge > 0:
                ...
                if _gc and 'halfspace' in _ct and _gc.get('halfspace_constraints'):
                    ...
                    for _cl_e, _cl_dash in _c_layers:
                        ...
                        for _hs3d in _gc['halfspace_constraints']:
                            ...
                            if _cl_dash:
                                # dashed quad edges only — no filled plane
    ...
    def predict(self, state, goal=None, extra_args=None, if_vision=False):
        ...
        Gen9 Ep2 Fix-3: avoiding = single camera, 2D positions.
        Visual:     state = (bp_image_np, des_xy_np, c_xy_np)
        Non-visual: state = obs_np  — Avoiding_Sim prepends des_xy to env obs
        """
        cond = None
        if if_vision:
            ...
            # bp_np is (C,H,W) float32 in BGR order; imageio expects RGB.
            if self.record_mode != 'none':
                try:
                    bp_vis = cv2.cvtColor(
        ...
        if self.action_counter == self.action_seq_size:
            ...
            # Gen9 Ep2 Fix-3: avoiding 6D traj [act(2)|des_xy(2)|c_xy(2)] → c_xy at 4:6.
            # Aligning had c_pos at 6:9 (9D traj); avoiding shifts it to 4:6 (6D traj).
            cpos_norm = traj_np[:, :, 4:6]   # (B, H, 2) normalized predicted c_xy positions
            if self.obs_normalizer is not None:
                B_f, H_f = cpos_norm.shape[:2]
                obs_dim = self.obs_normalizer.mins.shape[0]   # 4 (visual) or 20 (non-visual)
                dummy = np.zeros((B_f * H_f, obs_dim), dtype=np.float32)
                dummy[:, 2:4] = cpos_norm.reshape(-1, 2).astype(np.float32)
                obs_un = self.obs_normalizer.unnormalize(dummy)
                self.curr_rollout_all_candidates.append(obs_un[:, 2:4].reshape(B_f, H_f, 2).copy())
            else:
            ...
            # Gen9 Ep2 Fix-3: action dims = indices 0:2 (avoiding action_dim=2)
        ...
        self.step_counter   += 1
        return next_action_np.reshape(1, -1)   # (1, 2) — sim expects (1, act_dim) — Fix-3: 2D
    ...
    for seed in seeds:
        ...
        for geo_name, geo_config, geo_variant, is_tightened in _run_items:
            ...
            try:
                ...
                # FIX-18-followup: UF-13 used to indiscriminately flip if_vision=True
                # whenever recording was requested. That breaks for a genuinely
                # non-visual checkpoint (obs_normalizer dim 20) because the visual
                # predict() path then tries to normalize a 6-D state with a 20-D
