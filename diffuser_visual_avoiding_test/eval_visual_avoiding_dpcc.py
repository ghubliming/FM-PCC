# Visual-DPCC avoiding eval — U2 rebuild.
# Copy-modified from scripts/eval.py (DPCC diffuser baseline).
# Three targeted swaps vs the state-only baseline:
#   A) fm_visual_avoiding.utils  instead of diffuser.utils
...
RT_CONTROL_HZ = 30   # REAL_TIME_RECORDING_UPDATE — assumed deployment loop rate (budget=1000/hz ms); tune per target hardware. NB: total_ms bundles vision-encoder + diffusion + projection (agent.predict).
...
parser = argparse.ArgumentParser(description='Visual DPCC avoiding eval.')
    ...
    for halfspace_variant in halfspace_variants:
        ...
        for seed in seeds:
            ...
            if not args_cli.aggregate_only:
                ...
                # Trajectory dims — visual model is states_actions (action_dim=2)
            ...
            for variant_idx, variant in enumerate(projection_variants):
                ...
                if args_cli.aggregate_only:
                    ...
                    if not os.path.exists(npz_path):
                        print(f'[ eval ] skipping {variant} seed {seed}: no npz')
                        continue
                ...
                try:
                    ...
                    for i in range(n_trials):
                        ...
                        rt_rec = RTRecorder(episode_id=f'{exp}_{variant}_seed{seed}_trial{i}',
                                            ...
                                            system='VisualAvoiding_Diffuser',
                        ...
                        for _ in range(args.max_episode_length):
                            ...
                            if 'halfspace' in constraint_types:
                                for constraint in constraint_list_polytopic_not_tightened:
                                    if constraint[0] == 'ineq':
                                        c, d = constraint[1]
                                        obs_check = obs[:-diffusion.goal_dim] if diffusion.goal_dim > 0 else obs
                            ...
                            _rt_total_ms = (time.time() - start) * 1e3   # REAL_TIME_RECORDING_UPDATE — bundled encoder+diffusion+projection
